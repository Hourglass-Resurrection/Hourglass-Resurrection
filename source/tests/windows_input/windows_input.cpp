...
#define HANDLE_KEYBOARD_MESSAGE(msg)                            \
    ...
    {                                                           \
        ...
        oss.setf(std::ios_base::hex, std::ios_base::basefield); \
                                                                \
        oss << #msg L": wParam = " << wParam << L"; lParam = "  \
            << std::bitset<sizeof(lParam) * 8>(lParam);         \
                                                                \
...
#define HANDLE_MOUSE_MESSAGE(msg)                                                             \
    ...
    {                                                                                         \
        std::wostringstream oss;                                                              \
                                                                                              \
        oss << #msg L": wParam = " << std::bitset<sizeof(wParam) * 8>(wParam) << L"; xPos = " \
            << GET_X_LPARAM(lParam) << L"; yPos = " << GET_Y_LPARAM(lParam);                  \
                                                                                              \
        Logger::WriteLine(oss.str());                                                         \
        Input::LogData();                                                                     \
    }                                                                                         \
    ...
    case WM_MOUSEWHEEL:
    {
        ...
        oss << L"WM_MOUSEWHEEL: keystate = " << std::bitset<sizeof(keystate) * 8>(keystate)
            << L"; delta = " << delta << L"; xPos = " << GET_X_LPARAM(lParam) << L"; yPos = "
            << GET_Y_LPARAM(lParam);
