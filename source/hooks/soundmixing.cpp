...
#define clamptofullsignedrange(x, lo, hi)                                \
    (((unsigned int) ((x) - (lo)) <= (unsigned int) ((hi) - (lo))) ? (x) \
                                                                   : (((x) < 0) ? (lo) : (hi)))
...
static void Mix(const unsigned char* __restrict buf,
                unsigned char* __restrict outbuf,
                DWORD size,
                DWORD outSize,
                bool sizeReachesBufferEnd,
                Hooks::DirectSound::CachedVolumeAndPan& volumes)
{
    enum
    {
        fromshift = (2 + sizeof(fromtype) - sizeof(totype)) << 3
    }; // 16 when both buffers have the same bit/sample... this is for combining differing bitrates
    enum
    {
        maxto = (1 << (8 * sizeof(totype) - 1)) - 1
    }; // clamping magnitude (127 or 32767)
    enum
    {
        tosignoffset = (totype(-1) < 0) ? 0 : -(1 << (8 * sizeof(totype) - 1))
    }; // add this to make numbers in the "to" buffer signed
    enum
    {
        fromsignoffset = (fromtype(-1) < 0) ? 0 : -(1 << (8 * sizeof(fromtype) - 1))
    }; // add this to make numbers in the "from" buffer signed
    enum
    {
        toincrement = sizeof(totype) * tochannels
    };
    enum
    {
        fromincrement = sizeof(fromtype) * fromchannels
    };
    ...
    for (DWORD i = 0; i < outSize; outbuf += toincrement, i += toincrement)
    {
        ...
        offset -=
            offset
            % fromincrement; // prevent starting from the wrong speaker (compiler should be smart enough not to do a modulo op here)
        const unsigned char* inbuf = buf + offset;
        offset += fromincrement;
        if (sizeReachesBufferEnd
            && (offset > (size - fromincrement))) // take pos2IsLastSample into account
            offset =
                size
                - fromincrement; // note: don't subtract bufferSize instead, that doesn't work right (slight clicking) even in the looping case
        ...
        int mixedL = (otherL + ((int) (((myL * (int) lvas) >> fromshift) * (1024 - frac)
                                       + ((myL2 * (int) lvas) >> fromshift) * frac)
                                >> 10));
        int mixedR = (otherR + ((int) (((myR * (int) rvas) >> fromshift) * (1024 - frac)
                                       + ((myR2 * (int) rvas) >> fromshift) * frac)
                                >> 10));
        if (tochannels != 1)
        {
            // stereo output
            ((totype*) outbuf)[0] =
                (clamptofullsignedrange(mixedL, -maxto - 1, maxto)) - tosignoffset;
            ((totype*) outbuf)[1] =
                (clamptofullsignedrange(mixedR, -maxto - 1, maxto)) - tosignoffset;
        }
        else
        {
            // monaural output
...
static void Mix(const unsigned char* buf,
                ...
                Hooks::DirectSound::CachedVolumeAndPan& volumes)
{
    ...
    if (myBitsPerSample <= 8 && outBitsPerSample <= 8)
        Mix<unsigned char, unsigned char, fromchannels, tochannels>(buf,
                                                                    outbuf,
                                                                    size,
                                                                    outSize,
                                                                    sizeReachesBufferEnd,
                                                                    volumes);
    else if (myBitsPerSample > 8 && outBitsPerSample > 8)
        Mix<signed short, signed short, fromchannels, tochannels>(buf,
                                                                  outbuf,
                                                                  size,
                                                                  outSize,
                                                                  sizeReachesBufferEnd,
        ...
        Mix<unsigned char, signed short, fromchannels, tochannels>(buf,
                                                                   outbuf,
                                                                   size,
                                                                   outSize,
                                                                   sizeReachesBufferEnd,
        ...
        Mix<signed short, unsigned char, fromchannels, tochannels>(buf,
                                                                   outbuf,
                                                                   size,
                                                                   outSize,
                                                                   sizeReachesBufferEnd,
