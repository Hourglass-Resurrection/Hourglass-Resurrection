...
#define INTERNAL_HOOK_DEF(return_type, call_convention, name, my_name, ...)            \
    ...
    {                                                                                  \
        ...
        if (x__ == 2)                                                                  \
        {                                                                              \
            DEBUG_LOG() << "Function got called before it was set up!";                \
            _asm { int 3}                                                               \
            ;                                                                          \
        }                                                                              \
        x__++;                                                                         \
...
#define HOOK_ORDINAL(return_type, call_convention, target, decoration, ...) \
    INTERNAL_HOOK_DEF(return_type,                                          \
                      call_convention,                                      \
                      Ordinal##target##_##decoration,                       \
                      decoration,                                           \
                      ##__VA_ARGS__)
...
int offsetof_virtual(F f)
{
    ...
    while (*c++ != 0xFF)
    {
    } // search for jmp ff/4? (why don't x86 docs say what each combination of bytes does? I can't decode their gibberish tables.)
    switch (*c++)
    {
    case 0x60:
...
void HookCOMInterfaceEx(REFIID riid,
                        LPVOID* ppvOut,
                        REFGUID parameter,
                        bool uncheckedFastNew = false);
...
#define HOOKRIID(x, n)                                                                 \
    case IID_I##x##n##_Data1:                                                          \
        if (IID_I##x##n == riid                                                        \
            && (uncheckedFastNew || type_needs_hooking<I##x##n*, My##x<I##x##n>*>(     \
                                        reinterpret_cast<I##x##n*>(*ppvOut))))         \
        {                                                                              \
            if (!uncheckedFastNew && s_isDebug)                                        \
            {                                                                          \
                DEBUG_LOG() << "HOOKED COM INTERFACE: I" #x #n " (" << *ppvOut << ")"; \
            }                                                                          \
...
#define HOOKRIID2(x, my)                                                                     \
    case IID_I##x##_Data1:                                                                   \
        if (IID_I##x == riid && (uncheckedFastNew || type_needs_hooking<I##x*, my<I##x>*>(   \
                                                         reinterpret_cast<I##x*>(*ppvOut)))) \
        {                                                                                    \
            if (!uncheckedFastNew && s_isDebug)                                              \
            {                                                                                \
                DEBUG_LOG() << "HOOKED COM INTERFACE: I" #x " (" << *ppvOut << ")";          \
            }                                                                                \
...
#define HOOKRIID3(x, my)                                                                         \
    case IID_##x##_Data1:                                                                        \
        if (IID_##x == riid                                                                      \
            && (uncheckedFastNew || type_needs_hooking<x*, my*>(reinterpret_cast<x*>(*ppvOut)))) \
        {                                                                                        \
            if (!uncheckedFastNew && s_isDebug)                                                  \
            {                                                                                    \
                DEBUG_LOG() << "HOOKED COM INTERFACE: " #x " (" << *ppvOut << ")";               \
            }                                                                                    \
    ...
    case IID_##i##_Data1:                                                              \
        ...
        {                                                                              \
            ...
            {                                                                          \
                if (!uncheckedFastNew && s_isDebug)                                    \
                {                                                                      \
                    DEBUG_LOG() << "HOOKED COM INTERFACE: " #i " (" << *ppvOut << ")"; \
                }                                                                      \
            }                                                                          \
        }                                                                              \
        break
...
#define MAKE_INTERCEPT(enabled, dll, name)                                            \
    {                                                                                 \
        #dll ".dll", #name, (FARPROC) My##name, (FARPROC) Tramp##name, enabled, false \
    }
#define MAKE_INTERCEPT2(enabled, dll, name, myname)                                       \
    {                                                                                     \
        #dll ".dll", #name, (FARPROC) My##myname, (FARPROC) Tramp##myname, enabled, false \
    }
#define MAKE_INTERCEPT3(enabled, dllWithExt, name, suffix)                    \
    {                                                                         \
        #dllWithExt, #name, (FARPROC) My##name##_##suffix,                    \
                             (FARPROC) Tramp##name##_##suffix, enabled, false \
    }
#define MAKE_INTERCEPT_ORD(enabled, dll, name, ordinal)                                   \
    {                                                                                     \
        #dll ".dll",                                                                      \
            reinterpret_cast                                                              \
                < const char* > (ordinal),                                                \
                                 (FARPROC) My##name,                                      \
                                  (FARPROC) TrampOrdinal##ordinal##_##name, enabled, true \
    }
