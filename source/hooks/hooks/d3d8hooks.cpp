...
DEFINE_LOCAL_GUID(IID_IDirect3D8,
                  0x1DD9E8DA,
                  0x1C77,
                  0x4D40,
                  0xB0,
                  ...
                  0x95,
                  0x12);
DEFINE_LOCAL_GUID(IID_IDirect3DDevice8,
                  0x7385E5DF,
                  0x8FE8,
                  0x41D5,
                  0x86,
                  ...
                  0xB6,
                  0xCF);
DEFINE_LOCAL_GUID(IID_IDirect3DSwapChain8,
                  0x928C088B,
                  0x76B9,
                  0x4C6B,
                  0xA5,
                  ...
                  0x76,
                  0xCD);
DEFINE_LOCAL_GUID(IID_IDirect3DSurface8,
                  0xB96EEBCA,
                  0xB326,
                  0x4EA5,
                  0x88,
                  ...
                  0x21,
                  0xDD);
DEFINE_LOCAL_GUID(IID_IDirect3DTexture8,
                  0xE4CDD575,
                  0x2866,
                  0x4F01,
                  0xB1,
...
namespace Hooks
{
    ...
    static void ProcessPresentationParams8(D3DPRESENT_PARAMETERS* pPresentationParameters,
                                           IDirect3D8* d3d,
                                           IDirect3DDevice8* d3dDevice)
    {
        if (pPresentationParameters) // presentparams
        {
            pPresentationParameters->Flags |=
                D3DPRESENTFLAG_LOCKABLE_BACKBUFFER; // back buffer must be lockable, to allow for AVI recording (TODO: maybe that's not true anymore except if the usual method fails... if there's a significant performance penalty for this flag (which there might not be) then there should be an option to disable it)
            ...
            {
                FakeBroadcastDisplayChange(pPresentationParameters->BackBufferWidth,
    ...
    struct MyDirect3DDevice8
    {
        ...
        static ULONG STDMETHODCALLTYPE MyRelease(IDirect3DDevice8* pThis)
        {
            ...
            if (rv == 0)
            {
                ...
                {
                    pBackBufCopyOwner = NULL;
                }
                if (s_saved_d3d8Device == pThis)
                {
                    s_saved_d3d8Device = NULL;
                }
            }
            return rv;
        }

        static HRESULT(STDMETHODCALLTYPE* QueryInterface)(IDirect3DDevice8* pThis,
                                                          REFIID riid,
                                                          void** ppvObj);
        static HRESULT STDMETHODCALLTYPE MyQueryInterface(IDirect3DDevice8* pThis,
                                                          REFIID riid,
                                                          void** ppvObj)
        {
            ENTER();
        ...
        static HRESULT(STDMETHODCALLTYPE* Reset)(IDirect3DDevice8* pThis,
                                                 D3DPRESENT_PARAMETERS* pPresentationParameters);
        static HRESULT STDMETHODCALLTYPE MyReset(IDirect3DDevice8* pThis,
                                                 D3DPRESENT_PARAMETERS* pPresentationParameters)
        {
            ENTER();
        ...
        static void PresentFrameBoundary(IDirect3DDevice8* pThis,
                                         ...
                                         CONST RECT* pDestRect)
        {
            ...
            else
            {
                ...
                if (!pSurface)
                {
                    ...
                    if (SUCCEEDED(pThis->CreateImageSurface(d3ddesc.Width,
                                                            d3ddesc.Height,
                                                            d3ddesc.Format,
                                                            &pBackBufCopy)))
                    {
                ...
                if (pSurface != pBackBuffer)
                    if (FAILED(CopyRects(
                            pThis, pBackBuffer, pSourceRect, pSourceRect ? 1 : 0, pSurface, NULL)))
                        pSurface = pBackBuffer;
        ...
        static HRESULT(STDMETHODCALLTYPE* Present)(IDirect3DDevice8* pThis,
                                                   CONST RECT* pSourceRect,
                                                   CONST RECT* pDestRect,
                                                   HWND hDestWindowOverride,
                                                   CONST RGNDATA* pDirtyRegion);
        static HRESULT STDMETHODCALLTYPE MyPresent(IDirect3DDevice8* pThis,
                                                   CONST RECT* pSourceRect,
                                                   CONST RECT* pDestRect,
                                                   HWND hDestWindowOverride,
                                                   CONST RGNDATA* pDirtyRegion)
        {
            ...
            if ((d3d8BackBufActive || d3d8BackBufDirty) && !redrawingScreen)
            {
                ...
                if (pSourceRect)
                    s_savedD3D8SrcRect = *pSourceRect;
                if (pDestRect)
                    s_savedD3D8DstRect = *pDestRect;
                s_savedD3D8HWND =
                    hDestWindowOverride ? hDestWindowOverride : s_savedD3D8DefaultHWND;
        ...
        static HRESULT(STDMETHODCALLTYPE* CreateAdditionalSwapChain)(
            IDirect3DDevice8* pThis,
            D3DPRESENT_PARAMETERS* pPresentationParameters,
            IDirect3DSwapChain8** pSwapChain);
        static HRESULT STDMETHODCALLTYPE
        MyCreateAdditionalSwapChain(IDirect3DDevice8* pThis,
                                    D3DPRESENT_PARAMETERS* pPresentationParameters,
                                    IDirect3DSwapChain8** pSwapChain)
        {
            ENTER();
        ...
        static HRESULT(STDMETHODCALLTYPE* SetRenderTarget)(IDirect3DDevice8* pThis,
                                                           IDirect3DSurface8* pRenderTarget,
                                                           IDirect3DSurface8* pNewZStencil);
        static HRESULT STDMETHODCALLTYPE MySetRenderTarget(IDirect3DDevice8* pThis,
                                                           IDirect3DSurface8* pRenderTarget,
                                                           IDirect3DSurface8* pNewZStencil)
        {
            ENTER(pRenderTarget);
        ...
        static HRESULT(STDMETHODCALLTYPE* GetRenderTarget)(IDirect3DDevice8* pThis,
                                                           IDirect3DSurface8** ppRenderTarget);
        static HRESULT STDMETHODCALLTYPE MyGetRenderTarget(IDirect3DDevice8* pThis,
                                                           IDirect3DSurface8** ppRenderTarget)
        {
        ...
        static void Lock(IDirect3DDevice8* pThis,
                         DDSURFACEDESC& desc,
                         IDirect3DSurface8*& pBackBuffer,
                         CONST RECT* pSourceRect = NULL,
                         bool getBackBuffer = true)
        {
            ENTER();
            if (getBackBuffer)
                pThis->GetBackBuffer(0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer);
            ...
            D3DLOCKED_RECT lr;
            pBackBuffer->LockRect(&lr,
                                  0,
                                  D3DLOCK_NOSYSLOCK | D3DLOCK_READONLY | D3DLOCK_NO_DIRTY_UPDATE);
            ...
            {
                desc.dwWidth =
                    std::min<DWORD>(desc.dwWidth, (pSourceRect->right - pSourceRect->left));
                desc.dwHeight =
                    std::min<DWORD>(desc.dwHeight, (pSourceRect->bottom - pSourceRect->top));
            }
        ...
        static HRESULT(STDMETHODCALLTYPE* CopyRects)(IDirect3DDevice8* pThis,
                                                     IDirect3DSurface8* pSourceSurface,
                                                     CONST RECT* pSourceRectsArray,
                                                     UINT cRects,
                                                     IDirect3DSurface8* pDestinationSurface,
                                                     CONST POINT* pDestPointsArray);
        static HRESULT STDMETHODCALLTYPE MyCopyRects(IDirect3DDevice8* pThis,
                                                     IDirect3DSurface8* pSourceSurface,
                                                     CONST RECT* pSourceRectsArray,
                                                     UINT cRects,
                                                     IDirect3DSurface8* pDestinationSurface,
                                                     CONST POINT* pDestPointsArray)
        {
            ...
            {
                if (!d3d8BackBufDirty
                    && SUCCEEDED(pThis->GetBackBuffer(0, D3DBACKBUFFER_TYPE_MONO, &pBuffer)))
                {
        ...
        static HRESULT(STDMETHODCALLTYPE* Clear)(IDirect3DDevice8* pThis,
                                                 DWORD Count,
                                                 ...
                                                 float Z,
                                                 DWORD Stencil);
        static HRESULT STDMETHODCALLTYPE MyClear(IDirect3DDevice8* pThis,
                                                 DWORD Count,
                                                 CONST D3DRECT* pRects,
                                                 DWORD Flags,
                                                 D3DCOLOR Color,
        ...
        static HRESULT(STDMETHODCALLTYPE* DrawPrimitive)(IDirect3DDevice8* pThis,
                                                         D3DPRIMITIVETYPE PrimitiveType,
                                                         UINT StartVertex,
                                                         UINT PrimitiveCount);
        static HRESULT STDMETHODCALLTYPE MyDrawPrimitive(IDirect3DDevice8* pThis,
                                                         D3DPRIMITIVETYPE PrimitiveType,
                                                         UINT StartVertex,
                                                         UINT PrimitiveCount)
        {
        ...
        static HRESULT(STDMETHODCALLTYPE* DrawIndexedPrimitive)(IDirect3DDevice8* pThis,
                                                                D3DPRIMITIVETYPE primitiveType,
                                                                UINT minIndex,
                                                                UINT NumVertices,
                                                                UINT startIndex,
                                                                UINT primCount);
        static HRESULT STDMETHODCALLTYPE MyDrawIndexedPrimitive(IDirect3DDevice8* pThis,
                                                                D3DPRIMITIVETYPE primitiveType,
                                                                UINT minIndex,
                                                                UINT NumVertices,
                                                                UINT startIndex,
                                                                UINT primCount)
        {
            ...
            if (ShouldSkipDrawing(false, true))
                return D3D_OK;
            HRESULT rv = DrawIndexedPrimitive(pThis,
                                              primitiveType,
                                              minIndex,
                                              NumVertices,
                                              startIndex,
        ...
        static HRESULT(STDMETHODCALLTYPE* DrawPrimitiveUP)(IDirect3DDevice8* pThis,
                                                           D3DPRIMITIVETYPE PrimitiveType,
                                                           UINT PrimitiveCount,
                                                           CONST void* pVertexStreamZeroData,
                                                           UINT VertexStreamZeroStride);
        static HRESULT STDMETHODCALLTYPE MyDrawPrimitiveUP(IDirect3DDevice8* pThis,
                                                           D3DPRIMITIVETYPE PrimitiveType,
                                                           UINT PrimitiveCount,
                                                           CONST void* pVertexStreamZeroData,
                                                           UINT VertexStreamZeroStride)
        {
            ...
            if (ShouldSkipDrawing(false, true))
                return D3D_OK;
            HRESULT rv = DrawPrimitiveUP(pThis,
                                         PrimitiveType,
                                         PrimitiveCount,
        ...
        static HRESULT(STDMETHODCALLTYPE* DrawIndexedPrimitiveUP)(IDirect3DDevice8* pThis,
                                                                  D3DPRIMITIVETYPE PrimitiveType,
                                                                  UINT MinVertexIndex,
                                                                  ...
                                                                  CONST void* pVertexStreamZeroData,
                                                                  UINT VertexStreamZeroStride);
        static HRESULT STDMETHODCALLTYPE MyDrawIndexedPrimitiveUP(IDirect3DDevice8* pThis,
                                                                  D3DPRIMITIVETYPE PrimitiveType,
                                                                  UINT MinVertexIndex,
                                                                  UINT NumVertexIndices,
                                                                  UINT PrimitiveCount,
                                                                  ...
                                                                  UINT VertexStreamZeroStride)
        {
            ...
            if (ShouldSkipDrawing(false, true))
                return D3D_OK;
            HRESULT rv = DrawIndexedPrimitiveUP(pThis,
                                                PrimitiveType,
                                                MinVertexIndex,
                                                NumVertexIndices,
                                                PrimitiveCount,
        ...
        static HRESULT(STDMETHODCALLTYPE* CreateTexture)(IDirect3DDevice8* pThis,
                                                         UINT Width,
                                                         UINT Height,
                                                         UINT Levels,
                                                         DWORD Usage,
        ...
        static HRESULT STDMETHODCALLTYPE MyCreateTexture(IDirect3DDevice8* pThis,
                                                         UINT Width,
                                                         UINT Height,
                                                         UINT Levels,
                                                         DWORD Usage,
                                                         ...
                                                         IDirect3DTexture8** ppTexture)
        {
            ENTER(Width, Height, Pool);
            HRESULT rv =
                CreateTexture(pThis, Width, Height, Levels, Usage, Format, Pool, ppTexture);
            if (SUCCEEDED(rv))
            {
                ...
                for (int i = 0; i < numLevels; i++)
                {
                    ...
                    {
                        HookCOMInterface(IID_IDirect3DSurface8,
                                         reinterpret_cast<LPVOID*>(&pSurface));
        ...
        static HRESULT(STDMETHODCALLTYPE* CreateRenderTarget)(IDirect3DDevice8* pThis,
                                                              UINT Width,
                                                              UINT Height,
                                                              D3DFORMAT Format,
        ...
        static HRESULT STDMETHODCALLTYPE MyCreateRenderTarget(IDirect3DDevice8* pThis,
                                                              UINT Width,
                                                              UINT Height,
                                                              D3DFORMAT Format,
                                                              D3DMULTISAMPLE_TYPE MultiSample,
                                                              ...
                                                              IDirect3DSurface8** ppSurface)
        {
            ...
            Lockable = TRUE;
            HRESULT rv =
                CreateRenderTarget(pThis, Width, Height, Format, MultiSample, Lockable, ppSurface);
            if (SUCCEEDED(rv))
            {
    ...
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::QueryInterface)
    (IDirect3DDevice8* pThis, REFIID riid, void** ppvObj) = 0;
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::Present)
    (IDirect3DDevice8* pThis,
     CONST RECT* pSourceRect,
     CONST RECT* pDestRect,
     HWND hDestWindowOverride,
     CONST RGNDATA* pDirtyRegion) = 0;
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::Reset)
    (IDirect3DDevice8* pThis, D3DPRESENT_PARAMETERS* pPresentationParameters) = 0;
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::CreateAdditionalSwapChain)
    (IDirect3DDevice8* pThis,
     D3DPRESENT_PARAMETERS* pPresentationParameters,
     IDirect3DSwapChain8** pSwapChain) = 0;
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::SetRenderTarget)
    (IDirect3DDevice8* pThis,
     IDirect3DSurface8* pRenderTarget,
     IDirect3DSurface8* pNewZStencil) = 0;
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::GetRenderTarget)
    (IDirect3DDevice8* pThis, IDirect3DSurface8** ppRenderTarget) = 0;
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::CopyRects)
    (IDirect3DDevice8* pThis,
     IDirect3DSurface8* pSourceSurface,
     CONST RECT* pSourceRectsArray,
     UINT cRects,
    ...
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::DrawIndexedPrimitive)
    (IDirect3DDevice8* pThis,
     D3DPRIMITIVETYPE primitiveType,
     UINT minIndex,
     UINT NumVertices,
     UINT startIndex,
     UINT primCount) = 0;
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::Clear)
    (IDirect3DDevice8* pThis,
     DWORD Count,
     CONST D3DRECT* pRects,
     DWORD Flags,
     ...
     float Z,
     DWORD Stencil) = 0;
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::DrawPrimitive)
    (IDirect3DDevice8* pThis,
     D3DPRIMITIVETYPE PrimitiveType,
     UINT StartVertex,
     UINT PrimitiveCount) = 0;
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::DrawPrimitiveUP)
    (IDirect3DDevice8* pThis,
     D3DPRIMITIVETYPE PrimitiveType,
     UINT PrimitiveCount,
     CONST void* pVertexStreamZeroData,
     UINT VertexStreamZeroStride) = 0;
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::DrawIndexedPrimitiveUP)
    (IDirect3DDevice8* pThis,
     D3DPRIMITIVETYPE PrimitiveType,
     UINT MinVertexIndex,
     UINT NumVertexIndices,
    ...
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::CreateTexture)
    (IDirect3DDevice8* pThis,
     UINT Width,
     UINT Height,
     UINT Levels,
    ...
    HRESULT(STDMETHODCALLTYPE* MyDirect3DDevice8::CreateRenderTarget)
    (IDirect3DDevice8* pThis,
     UINT Width,
     UINT Height,
     D3DFORMAT Format,
    ...
    struct MyDirect3DSwapChain8
    {
        ...
        static ULONG STDMETHODCALLTYPE MyRelease(IDirect3DSwapChain8* pThis)
        {
            ...
            if (rv == 0)
            {
                if (s_saved_d3d8SwapChain == pThis)
                {
                    s_saved_d3d8SwapChain = NULL;
                }
            }
        ...
        static HRESULT(STDMETHODCALLTYPE* QueryInterface)(IDirect3DSwapChain8* pThis,
                                                          REFIID riid,
                                                          void** ppvObj);
        static HRESULT STDMETHODCALLTYPE MyQueryInterface(IDirect3DSwapChain8* pThis,
                                                          REFIID riid,
                                                          void** ppvObj)
        {
            ENTER();
        ...
        static HRESULT(STDMETHODCALLTYPE* Present)(IDirect3DSwapChain8* pThis,
                                                   CONST RECT* pSourceRect,
                                                   CONST RECT* pDestRect,
                                                   HWND hDestWindowOverride,
                                                   CONST RGNDATA* pDirtyRegion);
        static HRESULT STDMETHODCALLTYPE MyPresent(IDirect3DSwapChain8* pThis,
                                                   CONST RECT* pSourceRect,
                                                   CONST RECT* pDestRect,
                                                   HWND hDestWindowOverride,
                                                   CONST RGNDATA* pDirtyRegion)
    ...
    HRESULT(STDMETHODCALLTYPE* MyDirect3DSwapChain8::QueryInterface)
    (IDirect3DSwapChain8* pThis, REFIID riid, void** ppvObj) = 0;
    HRESULT(STDMETHODCALLTYPE* MyDirect3DSwapChain8::Present)
    (IDirect3DSwapChain8* pThis,
     CONST RECT* pSourceRect,
     CONST RECT* pDestRect,
     HWND hDestWindowOverride,
     CONST RGNDATA* pDirtyRegion) = 0;
    ...
    struct MyDirect3DSurface8
    {
        ...
        static ULONG STDMETHODCALLTYPE MyRelease(IDirect3DSurface8* pThis)
        {
            ...
            if (!rv)
            {
                ...
                if (!surf8
                         .isBackBuffer) // back buffer's ref count goes to 0 every frame? not sure why
                {
        ...
        static HRESULT(STDMETHODCALLTYPE* SetPrivateData)(IDirect3DSurface8* pThis,
                                                          REFGUID refguid,
                                                          CONST void* pData,
                                                          DWORD SizeOfData,
                                                          DWORD Flags);
        static HRESULT STDMETHODCALLTYPE MySetPrivateData(IDirect3DSurface8* pThis,
                                                          REFGUID refguid,
                                                          CONST void* pData,
                                                          DWORD SizeOfData,
                                                          DWORD Flags)
        ...
        static HRESULT(STDMETHODCALLTYPE* FreePrivateData)(IDirect3DSurface8* pThis,
                                                           REFGUID refguid);
        static HRESULT STDMETHODCALLTYPE MyFreePrivateData(IDirect3DSurface8* pThis,
                                                           REFGUID refguid)
        {
        ...
        static HRESULT(STDMETHODCALLTYPE* QueryInterface)(IDirect3DSurface8* pThis,
                                                          REFIID riid,
                                                          void** ppvObj);
        static HRESULT STDMETHODCALLTYPE MyQueryInterface(IDirect3DSurface8* pThis,
                                                          REFIID riid,
                                                          void** ppvObj)
        {
            ENTER(riid.Data1);
...
#define DEF(x, IMPL_ID)                                                                 \
    ...
    HRESULT(STDMETHODCALLTYPE* MyDirect3DSurface8<IMPL_ID>::QueryInterface)             \
    (x * pThis, REFIID riid, void** ppvObj) = 0;                                        \
    HRESULT(STDMETHODCALLTYPE* MyDirect3DSurface8<IMPL_ID>::UnlockRect)(x * pThis) = 0; \
    HRESULT(STDMETHODCALLTYPE* MyDirect3DSurface8<IMPL_ID>::SetPrivateData)             \
    (x * pThis, REFGUID refguid, CONST void* pData, DWORD SizeOfData, DWORD Flags) = 0; \
    HRESULT(STDMETHODCALLTYPE* MyDirect3DSurface8<IMPL_ID>::FreePrivateData)            \
    (x * pThis, REFGUID refguid) = 0;                                                   \
...
#undef HRESULT
    ...
    struct MyDirect3DTexture8
    {
        ...
        static HRESULT(STDMETHODCALLTYPE* SetPrivateData)(IDirect3DTexture8* pThis,
                                                          REFGUID refguid,
                                                          CONST void* pData,
                                                          DWORD SizeOfData,
                                                          DWORD Flags);
        static HRESULT STDMETHODCALLTYPE MySetPrivateData(IDirect3DTexture8* pThis,
                                                          REFGUID refguid,
                                                          CONST void* pData,
                                                          DWORD SizeOfData,
                                                          DWORD Flags)
        ...
        static HRESULT(STDMETHODCALLTYPE* FreePrivateData)(IDirect3DTexture8* pThis,
                                                           REFGUID refguid);
        static HRESULT STDMETHODCALLTYPE MyFreePrivateData(IDirect3DTexture8* pThis,
                                                           REFGUID refguid)
        {
            HRESULT hr = FreePrivateData(pThis, refguid);
            ENTER(pThis);
        ...
        static HRESULT(STDMETHODCALLTYPE* QueryInterface)(IDirect3DTexture8* pThis,
                                                          REFIID riid,
                                                          void** ppvObj);
        static HRESULT STDMETHODCALLTYPE MyQueryInterface(IDirect3DTexture8* pThis,
                                                          REFIID riid,
                                                          void** ppvObj)
        {
            ENTER(riid.Data1);
...
#define DEF(x)                                                                          \
    ...
    HRESULT(STDMETHODCALLTYPE* MyDirect3DTexture8::QueryInterface)                      \
    (x * pThis, REFIID riid, void** ppvObj) = 0;                                        \
    HRESULT(STDMETHODCALLTYPE* MyDirect3DTexture8::UnlockRect)(x * pThis, UINT) = 0;    \
    HRESULT(STDMETHODCALLTYPE* MyDirect3DTexture8::SetPrivateData)                      \
    (x * pThis, REFGUID refguid, CONST void* pData, DWORD SizeOfData, DWORD Flags) = 0; \
...
#undef HRESULT
    ...
    static void BackupVideoMemory8(IDirect3DSurface8* pThis)
    {
        ...
        if (SUCCEEDED(pThis->GetDesc(&desc)))
        {
            ...
            if (SUCCEEDED(pThis->LockRect(&lockedRect,
                                          NULL,
    ...
    void BackupVideoMemoryOfAllD3D8Surfaces()
    {
        ...
        IDirect3DSurface8* pBackBuffer;
        if (d3d8BackBufDirty && s_saved_d3d8Device
            && SUCCEEDED(
                   s_saved_d3d8Device->GetBackBuffer(0, D3DBACKBUFFER_TYPE_MONO, &pBackBuffer)))
        {
        ...
        for (std::map<IDirect3DTexture8*, IDirect3DTexture8_CustomData>::iterator iter =
                 texture8data.begin();
             iter != texture8data.end();
             iter++)
        {
        ...
        for (std::map<IDirect3DSurface8*, IDirect3DSurface8_CustomData>::iterator iter =
                 surface8data.begin();
             iter != surface8data.end();
             iter++)
        {
    ...
    bool RedrawScreenD3D8()
    {
        if (s_saved_d3d8Device)
        {
            ...
            if (pDstRect && !fakeDisplayValid)
            {
                ...
                dstRect.left = (dstRect.left - oldRect.left) * (newRect.right - newRect.left)
                                   / (oldRect.right - oldRect.left)
                               + newRect.left;
                dstRect.top = (dstRect.top - oldRect.top) * (newRect.bottom - newRect.top)
                                  / (oldRect.bottom - oldRect.top)
                              + newRect.top;
                dstRect.right = (dstRect.right - oldRect.left) * (newRect.right - newRect.left)
                                    / (oldRect.right - oldRect.left)
                                + newRect.left;
                dstRect.bottom = (dstRect.bottom - oldRect.top) * (newRect.bottom - newRect.top)
                                     / (oldRect.bottom - oldRect.top)
                                 + newRect.top;
            }
            ...
            if (s_saved_d3d8SwapChain)
                hr = s_saved_d3d8SwapChain->Present(s_savedD3D8pSrcRect,
                                                    pDstRect,
                                                    s_savedD3D8HWND,
                                                    NULL);
            else
                hr = s_saved_d3d8Device->Present(s_savedD3D8pSrcRect,
                                                 pDstRect,
                                                 s_savedD3D8HWND,
                                                 NULL);
            return true;
    ...
    struct MyDirect3D8
    {
        ...
        static HRESULT(STDMETHODCALLTYPE* QueryInterface)(IDirect3D8* pThis,
                                                          REFIID riid,
                                                          void** ppvObj);
        static HRESULT STDMETHODCALLTYPE MyQueryInterface(IDirect3D8* pThis,
                                                          REFIID riid,
                                                          void** ppvObj)
        {
            ENTER();
        ...
        static HRESULT(STDMETHODCALLTYPE* CreateDevice)(
            IDirect3D8* pThis,
            UINT Adapter,
        ...
        static HRESULT STDMETHODCALLTYPE
        MyCreateDevice(IDirect3D8* pThis,
                       UINT Adapter,
                       D3DDEVTYPE DeviceType,
                       HWND hFocusWindow,
                       DWORD BehaviorFlags,
    ...
    HRESULT(STDMETHODCALLTYPE* MyDirect3D8::QueryInterface)
    (IDirect3D8* pThis, REFIID riid, void** ppvObj) = 0;
    HRESULT(STDMETHODCALLTYPE* MyDirect3D8::CreateDevice)
    (IDirect3D8* pThis,
     UINT Adapter,
     D3DDEVTYPE DeviceType,
     HWND hFocusWindow,
