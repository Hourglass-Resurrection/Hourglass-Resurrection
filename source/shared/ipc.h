...
struct TasFlags
{
    ...
    std::array<bool,
               static_cast<std::underlying_type<LogCategory>::type>(
                   LogCategory::NUM_LOG_CATEGORIES)>
        log_categories;
#ifdef _USRDLL
...
namespace IPC
{
    ...
    public:
        ...
        {
        public:
            Iterator(const Message* msg) : m_msg(msg)
            {
            }

            const Iterator& operator++()
            {
                m_msg = reinterpret_cast<const Message*>(reinterpret_cast<const BYTE*>(m_msg)
                                                         + m_msg->size());
                return *this;
            }
