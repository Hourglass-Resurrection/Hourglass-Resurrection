...
#define JOY_FLAG \
    0x00100 // There are 8 joysticks supported, so the 7 next flags are also reserved for the other joysticks
#define XJOY_FLAG \
    0x10000 // There are 4 xinput joysticks supported, so the 3 next flags are also reserved for the other xinput joysticks
...
struct CurrentInput
{
    ...
    int serialize(unsigned char* input_pack)
    {
        ...
        for (int i = 1; i < 256; i++)
        { // We are starting at 1 because 0 is not a valid key code.
            if (keys[i] != 0)
            {
        ...
        unsigned char mouse_packed
            [21]; // 4 bytes for each relative direction (X, Y, Z), 1 byte for the buttons and 8 bytes for the aboslute coords.
        if (isMouseUsed)
        {
        ...
        for (int i = 0; i < 4; i++)
        {
            isXControllerUsed[i] = (gamepad[i].wButtons != 0) || (gamepad[i].bLeftTrigger != 0)
                                   || (gamepad[i].bRightTrigger != 0) || (gamepad[i].sThumbLX != 0)
                                   || (gamepad[i].sThumbLY != 0) || (gamepad[i].sThumbRX != 0)
                                   || (gamepad[i].sThumbRY != 0);
    ...
    int unserialize(unsigned char* packed_input)
    {
        ...
        {
            if (mask & (XJOY_FLAG << i))
            { // The i-th XController is present.
