...
private:
    ...
    static std::map<SingleInput, SingleInput>
        inputMapping; // mapping from a single input to a single input.
    ...
    unsigned char
        oldKeys[DI_KEY_NUMBER]; // List of previous key states, needed for events triggering.
