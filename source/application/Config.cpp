...
namespace Config
{
    ...
    TasFlags localTASflags = {
        ...
        2, //audioChannels
        0, //stateLoaded
        FFMODE_FRONTSKIP | FFMODE_BACKSKIP | FFMODE_RAMSKIP
            | FFMODE_SLEEPSKIP, //fastForwardFlags,// | (recoveringStale ? (FFMODE_FRONTSKIP|FFMODE_BACKSKIP) ? 0),
        6000, // initialTime
        1, //timescale
        1, //timescaleDivisor
    ...
    bool nextLoadRecords =
        true; // false if next load switches to playback, true if next load switches to recording... aka readonly/read-only toggle
    ...
    int inputFocusFlags =
        FOCUS_FLAG_TASEE | FOCUS_FLAG_OTHER | FOCUS_FLAG_TASER; // allowbackgroundinput;
    ...
    int Save_Config(LPCWSTR filename)
    {
        ...
        else
        {
            swprintf(Conf_File,
                     ARRAYSIZE(Conf_File),
                     L"%s\\%s",
                     this_process_path.c_str(),
                     filename);
        }

        //SetPrivateProfileIntA("Graphics", "Force Windowed", localTASflags.forceWindowed, Conf_File);
        SetPrivateProfileIntW(L"Tools",
                              L"Fast Forward Flags",
                              localTASflags.fastForwardFlags,
                              Conf_File);
        if (advancePastNonVideoFramesConfigured)
        {
            SetPrivateProfileIntW(L"Input",
                                  L"Skip Lag Frames",
                                  advancePastNonVideoFrames,
                                  Conf_File);
        }
        SetPrivateProfileIntW(L"Input",
                              L"Background Input Focus Flags",
                              inputFocusFlags,
                              Conf_File);
        ;
        SetPrivateProfileIntW(L"Input",
                              L"Background Hotkeys Focus Flags",
                              hotkeysFocusFlags,
                              Conf_File);
    ...
    int Load_Config(LPCWSTR filename)
    {
        ...
        localTASflags.fastForwardFlags = GetPrivateProfileIntW(L"Tools",
                                                               L"Fast Forward Flags",
                                                               localTASflags.fastForwardFlags,
                                                               Conf_File);
        advancePastNonVideoFrames = GetPrivateProfileIntW(L"Input",
                                                          L"Skip Lag Frames",
                                                          advancePastNonVideoFrames,
                                                          Conf_File);
        advancePastNonVideoFramesConfigured =
            0 != GetPrivateProfileIntW(L"Input", L"Skip Lag Frames", 0, Conf_File);
        inputFocusFlags = GetPrivateProfileIntW(L"Input",
                                                L"Background Input Focus Flags",
                                                inputFocusFlags,
                                                Conf_File);
        hotkeysFocusFlags = GetPrivateProfileIntW(L"Input",
                                                  L"Background Hotkeys Focus Flags",
                                                  hotkeysFocusFlags,
                                                  Conf_File);

        traceEnabled =
            0 != GetPrivateProfileIntW(L"Debug", L"Load Debug Tracing", traceEnabled, Conf_File);
