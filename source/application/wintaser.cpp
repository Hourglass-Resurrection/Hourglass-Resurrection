...
#ifndef CONTEXT_ALL
#define CONTEXT_ALL \
    (CONTEXT_FULL | CONTEXT_FLOATING_POINT | CONTEXT_DEBUG_REGISTERS | CONTEXT_EXTENDED_REGISTERS)
#endif
...
void DetectWindowsVersion(int* major, int* minor)
{
    ...
    if (value != 0)
    {
        ...
        if (info_size != 0)
        {
            ...
            {
                BOOL parse_success =
                    VerQueryValueW(ver_info, L"\\", reinterpret_cast<LPVOID*>(&buffer), &size);
                if (parse_success != FALSE && size > 0)
                {
                    if (buffer->dwSignature == 0xFEEF04BD)
                    {
                        ...
                        *minor = LOWORD(buffer->dwFileVersionMS);
                        ...
                        DebugLog()
                            << "Detection succeeded, detected version " << *major << "." << *minor;
                        return;
                    }
                }
            }
            delete
                [] ver_info; // Destroy this if we failed, we cannot have a memory leak now can we?
        }
    }
...
bool IsWindowsXP()
{
    return localTASflags.osVersionMajor == 5
           && (localTASflags.osVersionMinor == 1 || localTASflags.osVersionMinor == 2);
}
bool IsWindowsVista()
{
    return localTASflags.osVersionMajor == 6 && localTASflags.osVersionMinor == 0;
}
bool IsWindows7()
{
    return localTASflags.osVersionMajor == 6 && localTASflags.osVersionMinor == 1;
}
// Not used for anything yet, but at least the function is here now, just in case.
bool IsWindows8()
{
    return localTASflags.osVersionMajor == 6
           && (localTASflags.osVersionMinor == 2 || localTASflags.osVersionMinor == 3);
}
...
std::wstring NormalizePath(const std::wstring& path)
{
    ...
    if (len != 0 && len < MAX_PATH)
    {
        GetLongPathNameW(
            normalized_path,
            normalized_path,
            MAX_PATH); // GetFullPathName won't always convert short filenames to long filenames
    }
            ...
            *drive = *p;
            if (QueryDosDeviceW(drive, name, MAX_PATH) != 0)
            {
                ...
                if (name_len < MAX_PATH)
                {
                     ...
                     */
                    if (filename.find(L"\\Device\\Mup")
                        == 0) // HACK! - Symbolic links can't be resolved properly for \\Device\\Mup paths.
                    {
                    ...
                    if (found)
                    {
                        ...
                        int len = swprintf(temp_file,
                                           MAX_PATH,
                                           L"%s%s",
                                           drive,
                                           filename.c_str() + name_len);
...
struct ThreadInfo
{
    ...
    {
        ASSERT(h);
        handle = h; /*waitingCount = 0; hopelessness = 0;*/
        name[0] = 0;
        return *this;
    }
};
...
void CheckSrcDllVersion(DWORD version)
{
    if (version != VERSION)
    {
        LPCWSTR str;
        if (version > VERSION)
            str = L"Wrong version detected: This hourglass.exe is too old compared to "
                  L"hooks.dll.\nPlease make sure you have extracted Hourglass properly.\nThe files "
                  L"that came with hourglass.exe must stay together with it.";
        else
            str = L"Wrong version detected: hooks.dll is too old compared to this "
                  L"hourglass.exe.\nPlease make sure you have extracted Hourglass properly.\nThe "
                  L"files that came with hourglass.exe must stay together with it.";
        DebugLog() << str;
...
int LoadMovie(const std::wstring& filename)
{
    ...
    if (rv == false)
        return 0; // Check if LoadMovieFromFile failed, if it did we don't need to continue.
    ...
    if (localTASflags.playback)
    {
        ...
        {
            CustomMessageBox(
                L"This movie file doesn't contain any frames, playback is not possible.",
                L"Error!",
                (MB_OK | MB_ICONERROR));
        ...
        if (movie.version != VERSION)
        {
            ...
            swprintf(str,
                     ARRAYSIZE(str),
                     L"This movie was recorded using a different main version of Hourglass "
                     L"Resurrection.\n"
                     L"\n"
                     L"Movie's version: %u\n"
                     L"Program version: %u\n"
                     ...
                     L"If it would desync you might want to use the movies main version of "
            ...
            int result = CustomMessageBox(
                str,
                L"Warning!",
                (MB_YESNO | MB_ICONWARNING
                 | MB_DEFBUTTON1)); // MS_DEFBUTTON1 marks "Yes" as the default choice.
            if (result == IDNO)
            {
                return -1;
            }
        ...
        if (!Arguments::g_args.m_ignore_md5)
        {
            ...
            if (memcmp(movie.fmd5, temp_md5, 4 * 4) != 0)
            {
                ...
                swprintf(str,
                         ...
                         CalcExeFilesize());
                int result =
                    CustomMessageBox(str, L"Warning!", (MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON2));
                if (result == IDNO)
                {
                    return -1;
                }
        ...
        if (movie.it != localTASflags.initialTime)
        {
            ...
            swprintf(
                str,
                ARRAYSIZE(str),
                L"This movie was recorded using a different initial time.\n\n"
                ...
                L"(Click \"Yes\" to use the movies initial time, \"No\" to use current initial "
                L"time)",
                static_cast<int>(movie.it),
                localTASflags.initialTime);
            int result =
                CustomMessageBox(str, L"Warning!", (MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON1));
            if (result == IDYES)
            {
        ...
        if (wcscmp(movie.commandline, command_line.c_str()) != 0)
        {
            ...
            swprintf(str,
                     ARRAYSIZE(str),
                     L"This movie was recorded using a different command line.\n\nMovie's command "
                     L"line: %s\nCurrent command line: %s\n\nPlaying the movie with current "
                     L"command line may lead to the movie desyncing.\nDo you want to use the "
                     L"movies command line instead?\n(Click \"Yes\" to use the movies command "
                     L"line, \"No\" to use current command line)",
                     movie.commandline,
                     command_line.c_str());
            int result =
                CustomMessageBox(str, L"Warning!", (MB_YESNO | MB_ICONWARNING | MB_DEFBUTTON1));
...
void SaveGameStatePhase2(int slot)
{
    ...
    if (finished && !recoveringStale)
    {
        ...
        LPCWSTR str =
            L"Warning: The movie is in \"Finished\" status.\n"
        ...
        int result =
            CustomMessageBox(str, L"Desync Warning", MB_YESNO | MB_DEFBUTTON2 | MB_ICONWARNING);
        if (result == IDNO)
            return;
    }
    ...
    // save all the threads
    {
        ...
        for (iter = hGameThreads.begin(); iter != hGameThreads.end(); iter++)
        {
            ...
            if (thread.suspendCount == -1)
                DebugLog() << "ERROR: Failed to suspend thread?? hThread=" << std::hex
                           << thread.handle << ", lastError=" << GetLastError();
            else
            {
                ...
                if (GetThreadContext(thread.handle, &thread.context))
                    ...
                    DebugLog() << "ERROR: Failed to save thread?? hThread=" << std::hex
                               << thread.handle << ", lastError=" << GetLastError();
            }
        }
    }
    ...
    // save all the memory
    {
        ...
        while (lpMem < si.lpMaximumApplicationAddress)
        {
                ...
                && (mbi.State & MEM_COMMIT))
            {
                ...
                if (ReadProcessMemory(hGameProcess,
                                      mbi.BaseAddress,
                                      region.data,
                                      1,
                                      &bytesRead) // save 1 byte so we can test it
                    && WriteProcessMemory(hGameProcess,
                                          mbi.BaseAddress,
                                          region.data,
                                          1,
                                          &bytesRead) // (testing it here speeds up saves)
                ...
                {
                    VerboseDebugLog() << "WARNING: couldn't save memory: baseAddress=" << std::hex
                                      << mbi.BaseAddress << ", regionSize=" << mbi.RegionSize
    ...
    // print some memory usage info
    {
        ...
        DebugLog() << numValidStates << " savestates, " << (totalBytes / (float) (1024 * 1024))
                   << " MB logical, " << (totalUniqueBytes / (float) (1024 * 1024)) << " MB actual";
    }
}
...
static bool MovieStatePreceeds(const Movie& shorter, const Movie& longer)
{
    if (shorter.currentFrame > longer.currentFrame)
        return false;
    if (memcmp(&shorter.frames[0],
               &longer.frames[0],
               shorter.currentFrame * sizeof(shorter.frames[0])))
        return false;
    return true;
}
...
void LoadGameStatePhase2(int slot)
{
    ...
    if (!wasRecoveringStale)
    {
        ...
        // load all the threads
        {
            ...
            for (unsigned int i = 0; i < state.threads.size(); i++)
            {
                ...
                if (hGameThreads[thread.id] != thread.handle)
                {
                    ...
                    DebugLog() << "FAILED TO LOAD THREAD: id=" << std::hex << thread.id
                ...
                VerboseDebugLog() << "Loaded thread: handle=" << std::hex << thread.handle
                                  << ", id=" << thread.id
        ...
        // load all the memory (instant brain transplant)
        {
            for (unsigned int i = 0; i < state.memory.size(); i++)
            {
                ...
                VerboseDebugLog() << "LMEM: BaseAddress=" << std::hex << mbi.BaseAddress
                                  << ", AllocationBase=" << mbi.AllocationBase
                                  << ", AllocationProtect=" << mbi.AllocationProtect
                                  << ", RegionSize=" << mbi.RegionSize << ", State=" << mbi.State
                ...
                void* allocAddr = VirtualAllocEx(hGameProcess,
                                                 mbi.BaseAddress,
                                                 mbi.RegionSize,
                                                 mbi.State,
                                                 mbi.Protect & ~PAGE_GUARD);
                if (allocAddr != mbi.BaseAddress && mbi.State == MEM_COMMIT
                    && GetLastError()
                           != 0x5) // NOTE: Replace with ERROR_ACCESS_DENIED for the sake of cleaner code?
                    allocAddr = VirtualAllocEx(hGameProcess,
                                               mbi.BaseAddress,
                                               mbi.RegionSize,
                ...
                if (allocAddr != mbi.BaseAddress && !(mbi.Type & MEM_IMAGE))
                    DebugLog() << "FAILED TO COMMIT MEMORY REGION: BaseAddress=" << std::hex
                               << mbi.BaseAddress << ", AllocationBase=" << mbi.AllocationBase
                               << ", RegionSize=" << mbi.RegionSize
                               << ", ResultAddress=" << allocAddr
                               << ", LastError=" << GetLastError();
                DWORD dwOldProtect = 0;
                BOOL protectResult = VirtualProtectEx(hGameProcess,
                                                      mbi.BaseAddress,
                                                      mbi.RegionSize,
                                                      mbi.Protect & ~PAGE_GUARD,
                                                      &dwOldProtect);
                if (!protectResult) //&& !(mbi.Type & MEM_IMAGE))
                    DebugLog() << "FAILED TO PROTECT MEMORY REGION: BaseAddress=" << std::hex
                               << mbi.BaseAddress << ", RegionSize=" << mbi.RegionSize
                               << ", LastError=" << GetLastError();

                SIZE_T bytesWritten = 0;
                BOOL writeResult = WriteProcessMemory(hGameProcess,
                                                      mbi.BaseAddress,
                                                      region.data,
                                                      mbi.RegionSize,
                                                      &bytesWritten);
                if (!writeResult) //&& !(mbi.Type & MEM_IMAGE))
                    DebugLog() << "FAILED TO WRITE MEMORY REGION: BaseAddress=" << std::hex
                               << mbi.BaseAddress << ", RegionSize=" << mbi.RegionSize
                               << ", LastError=" << GetLastError();
    ...
    if (nextLoadRecords || wasRecoveringStale) // switch to recording / read+write
    {
        ...
        movie.rerecordCount =
            (unsavedMovieData || localTASflags.playback) ? (rerecords + 1) : (rerecords);
    ...
    else // switch to playback / read-only
    {
        ...
        {
            finished = true;
            DebugLog() << "MOVIE END WARNING: loaded movie frame " << state.movie.currentFrame
                       << " comes after end of current movie length " << movie.frames.size();
            bool warned = false;
            if (!warned)
            {
                warned = true;
                ...
                swprintf(str,
                         ARRAYSIZE(str),
                         L"Warning: Loaded state is at frame %d, but current movie is only %d "
                         L"frames long.\n"
                         L"You should load a different savestate before continuing, or switch to "
                         L"read+write and reload it.",
                         state.movie.currentFrame,
                         static_cast<int>(movie.frames.size()));
            ...
            for (int i = 0; i < movie.currentFrame; i++)
            {
                if (memcmp(&movie.frames[i], &state.movie.frames[i], sizeof(MovieFrame)))
                {
                    ...
                    if (!warned)
                    {
                        ...
                        swprintf(str,
                                 ARRAYSIZE(str),
                                 L"Warning: Loaded state's movie input does not match current "
                                 L"movie input on frame %d.\n"
                                 L"This can cause a desync. You should load a different savestate "
                                 L"before continuing.",
                                 i);
...
void SendCommand()
{
    if (requestedCommandReenter) // a second SendCommand can overwrite the first, which is probably an error
    {
        ...
        ReadProcessMemory(hGameProcess,
                          remoteCommandSlot,
                          tempCommandSlot,
                          sizeof(tempCommandSlot),
                          &bytesRead);
        DebugLog() << __FUNCTIONW__
                   << " ERROR!!! overwrote earlier command before it executed. replaced \""
                   << tempCommandSlot << "\" with \"" << localCommandSlot << "\".";
...
void SendTASFlags()
{
    ...
    localTASflags.emuMode =
        localTASflags.emuMode
        | (((recoveringStale || (localTASflags.fastForwardFlags & FFMODE_SOUNDSKIP))
            && localTASflags.fastForward)
               ? EMUMODE_NOPLAYBUFFERS
               : 0)
        | ((localTASflags.threadMode == 0 || localTASflags.threadMode == 4
            || localTASflags.threadMode == 5)
               ? EMUMODE_VIRTUALDIRECTSOUND
               : 0);
...
void RecordLocalInputs()
{
    ...
    {
        finished = true;
        DebugLog() << "RECORDING STOPPED because " << movie.currentFrame
                   << " != " << movie.frames.size();
    }
}
...
void InjectCurrentMovieFrame() // playback ... or recording, now
{
    ...
    SIZE_T bytesWritten = 0;
    if (!WriteProcessMemory(hGameProcess,
                            remoteInputs,
                            &frame.inputs,
                            sizeof(CurrentInput),
                            &bytesWritten))
...
void AddAndSendDllInfo(LPCWSTR filename, bool loaded, HANDLE hProcess)
{
    ...
    if (dllLoadInfos.numInfos < (int) ARRAYSIZE(dllLoadInfos.infos) + (filename ? 0 : 1))
    {
        ...
        if (filename)
        {
            ...
            memmove(&dllLoadInfos.infos[1],
                    dllLoadInfos.infos,
                    dllLoadInfos.numInfos * sizeof(*dllLoadInfos.infos));
...
void UpdateGeneralInfoDisplay()
{
    if (remoteGeneralInfoFromDll)
    {
        ...
        SIZE_T bytesRead = 0;
        if (ReadProcessMemory(hGameProcess,
                              remoteGeneralInfoFromDll,
                              &localGeneralInfoFromDll,
                              sizeof(InfoForDebugger),
                              &bytesRead))
        {
            WCHAR str[256];
            WCHAR str2[256];
...
void CheckDialogChanges(int frameCount)
{
    if (!displayed_checkdialog_inited || started != displayed_started
        ...
        || finished != displayed_finished)
    {
        ...
        EnableWindow(GetDlgItem(hWnd, IDC_AVIAUDIO),
    ...
    if (!displayed_checkdialog_inited
        || static_cast<int>(displayed_fastforward) != localTASflags.fastForward)
    {
        displayed_fastforward = (localTASflags.fastForward != 0);
        CheckDlgButton(hWnd, IDC_FASTFORWARD, localTASflags.fastForward);
    }
    if (!displayed_checkdialog_inited
        || displayed_usedThreadMode != usedThreadMode && usedThreadMode >= 0)
    {
    ...
    if (!displayed_checkdialog_inited || displayed_threadMode != localTASflags.threadMode
        || displayed_waitSyncMode != localTASflags.waitSyncMode)
    {
        ...
        CheckDlgButton(hWnd,
                       ...
                       localTASflags.threadMode == 0 && localTASflags.waitSyncMode == 1);
        CheckDlgButton(hWnd,
                       IDC_RADIO_THREAD_WRAP,
                       localTASflags.threadMode == 1 && localTASflags.waitSyncMode == 1);
        CheckDlgButton(hWnd,
                       IDC_RADIO_THREAD_ALLOW,
                       localTASflags.threadMode == 2 && localTASflags.waitSyncMode == 2);
    }
...
void FrameBoundary(IPC::FrameBoundaryInfo& frame_info, int threadId)
{
    if (!requestedCommandReenter)
    {
        ...
        if (frame_info.GetCaptureInfoType() != CAPTUREINFO_TYPE_NONE_SUBSEQUENT)
        {
            ...
            if (localTASflags.aviMode & 1)
            {
                ProcessCaptureFrameInfo(frame_info.GetCaptureInfo(),
                                        frame_info.GetCaptureInfoType());
            }
            if (localTASflags.aviMode & 2)
            {
                ProcessCaptureSoundInfo();
        ...
        temporaryUnpause =
            (frame_info.GetCaptureInfoType() == CAPTUREINFO_TYPE_PREV) && advancePastNonVideoFrames;
    }
    else
    {
    ...
    do
    {
        ...
        if (paused)
        {
            ...
            if (!temporaryUnpause
                && !requestedCommandReenter) // check to avoid breaking... everything
            {
    ...
    if (which < 16 && !finished)
    {
        if (remoteGeneralInfoFromDll)
        {
                                  ...
                                  &bytesRead))
            {
                ...
                {
                    if (localTASflags.playback && movieTime
                        && s_firstTimerDesyncWarnedFrame >= frameCount)
                    {
                        ...
                        swprintf(str,
                                 ARRAYSIZE(str),
                                 L"DESYNC DETECTED: on frame %d, your timer = %d but movie's timer "
                                 L"= %d.\nThat means this playback of the movie desynced somewhere "
                                 L"between frames %d and %d.",
                                 frameCount,
                                 localGeneralInfoFromDll.ticks,
                                 movieTime,
                                 frameCount,
...
void SuspendAllExcept(int ignoreThreadID)
{
    ...
    if (!ok)
    {
        ...
        if (first && ignoreThreadID)
            DebugLog() << "ERROR: suspendall failed to find requesting thread id=" << std::hex
                       << ignoreThreadID << " in list.";
        first = 0;
    }
}
void ResumeAllExcept(int ignoreThreadID)
{
    ...
    if (!ok)
    {
        static int first = 1;
        if (first)
            DebugLog() << "ERROR: resumeall failed to find requesting thread id=" << std::hex
                       << ignoreThreadID << " in list.";
        first = 0;
    }
}
...
std::wstring ExceptionCodeToDescription(DWORD code)
{
     ...
     */
    case /*DELPHI_RUNTIME_ERROR*/ 0x0EEDFADE:
        return L"Delphi runtime error.";
    case /*EXCEPTION_MSVC*/ 0xC06D7363:
        return L"exception inserted by Visual C++, often used for SEH";
    case /*EXCEPTION_COMPLUS*/ 0xC0524F54:
        return L"EXCEPTION_COMPLUS (ROT)";
    case /*EXCEPTION_COMPLUS*/ 0xC0434352:
        return L"EXCEPTION_COMPLUS (CCR)";
    case /*EXCEPTION_COMPLUS*/ 0xC0434F4D:
        return L"EXCEPTION_COMPLUS (COM)";
    case /*EXCEPTION_HIJACK*/ 0xC0434F4E:
        return L"Resuming thread after garbage collection.";
    case /*EXCEPTION_SOFTSO*/ 0xC053534F:
        return L"Stack overflow in managed code.";
    case /*EXCEPTION_EXX*/ 0xC0455858:
        return L"SetupThread PInvoke failure.";
    case 0xC0440001:
        return L"D runtime Win32 exception";
    default:
        std::wstring msg;
...
#endif
                CloseHandle(
                    handle); // if we get a EXCEPTION_INVALID_HANDLE exception here that's really bad... maybe the LdrLoadDll hook is doing too much and so we got debugging events with wrong thread handles... or maybe we called CloseHandle on the wrong things in the debug event handlers here
#ifndef _DEBUG
            }
            catch (const Utils::Exceptions::WindowsException& e)
            {
                DebugLog() << "Exception while closing thread " << std::hex << handle << ": "
                           << e.what();
            }
#endif
        }
...
static void HandleThreadExitEvent(DEBUG_EVENT& de, PROCESS_INFORMATION& processInfo)
{
    ...
    if (found != hGameThreads.end())
    {
        ...
        DebugLog() << "STOPPED THREAD: id=" << std::hex << de.dwThreadId << ", handle=" << hThread
    ...
    std::vector<DWORD>::iterator found2 =
        std::find(gameThreadIdList.begin(), gameThreadIdList.end(), de.dwThreadId);
...
static DWORD WINAPI DebuggerThreadFunc(LPVOID lpParam)
{
	...
	if(!EnableDebugPrivilege())
	{
        ...
        BOOL created = CreateProcessW(
            exe_filename.c_str(), // application name
        ...
        if (!created)
        {
            ...
            if (error == ERROR_ELEVATION_REQUIRED)
            {
                ...
                swprintf(
                    ...
                    L"Try closing Hourglass and then opening it with \"Run as administrator\".",
                    exe_filename.c_str());
        ...
        DebugLog() << "done creating process, got handle " << std::hex << hGameProcess
        ...
        if (limitGameToOnePhysicalCore) // We want to limit the game to one physical core to avoid issues
        {
            ...
            if (logicalCores > 1)
            {
                unsigned int numHyperThreadCores =
                    logicalCores
                    / physicalCores; // Get how many logical cores each physical CPU core consists of.
                ...
                {
                    PrintLastError(L"SetProcessAffinityMask",
                                   GetLastError()); // We failed, write the reason to the error log.
                }
            }
        }
        ...
        else if (disableHyperThreading)
        {
            ...
            {
                unsigned int numHyperThreadCores =
                    logicalCores / physicalCores; // Number of Hyper-Thread cores per physical core.
...
#pragma region OUTPUT_DEBUG_STRING_EVENT
                    ...
                    char* str =
                        new char[len]; // Taking all messages instead of ignoring extra large ones.
                    //if(len+1 < sizeof(str)) // just ignore any strings bigger than our buffer size
                    {
                        SIZE_T bytesRead = 0;
                        if (!ReadProcessMemory(hProcess,
                                               dsi.lpDebugStringData,
                                               (LPVOID) str,
                                               len,
                                               &bytesRead))
                            ::sprintf(str,
                                      /*"SHORTTRACE: 0,50 "*/ "ERROR READING STRING of length %d\n",
                                      len);

                        const char* pstr = str;

#define MessagePrefixMatch(pre) \
    (!strncmp(pstr, pre ": ", sizeof(pre ": ") - 1) ? pstr += sizeof(pre ": ") - 1 : false)
                        if (MessagePrefixMatch("IPCBUFFER"))
                        {
                            ipc_address_in_process =
                                reinterpret_cast<LPVOID>(strtol(pstr, nullptr, 16));
                        }
...
#undef MessagePrefixMatch
                    ...
                    delete
                        [] str; // destroying str since it's now a pointer, and we don't need it anymore.
#pragma endregion
                }
                break;
...
#pragma region EXCEPTION_DEBUG_EVENT
                    ...
                    if (de.u.Exception.ExceptionRecord.ExceptionCode == EXCEPTION_BREAKPOINT)
                    {
                        ...
                        {
                            DebugLog()
                                << "hit custom breakpoint at address = " << std::hex << address;
                            if (address == entrypoint)
                            {
                                RemoveBreakpoint(address,
                                                 de.dwThreadId,
                                                 /*GetProcessHandle(processInfo,de)*/ hGameProcess);
                                ...
                                {
                                    DebugLog()
                                        << "suspending main thread until PostDllMain finishes...";
                         ...
                         * -- Warepire
                         */
                            BOOL rv = ReadProcessMemory(hGameProcess,
                                                        ipc_address_in_process,
                                                        &ipc_frame,
                                                        sizeof(ipc_frame),
                                                        &read_bytes);
                            if (rv && read_bytes == sizeof(ipc_frame))
                            {
                                if (ipc_frame.m_command > IPC::Command::CMD_DUMMY_ENTRY_MIN_OPCODE
                                    && static_cast<std::underlying_type<IPC::Command>::type>(
                                           ipc_frame.m_command)
                                               % 2
                                           == 0)
                                {
                                    ...
                                    if (ipc_frame.m_command_data_size != 0)
                                    {
                                        ...
                                        rv = ReadProcessMemory(hGameProcess,
                                                               ipc_frame.m_command_data,
                                                               buf.data(),
                                                               buf.capacity(),
                                                               &read_bytes);
                                    }
                                    if (rv && read_bytes == ipc_frame.m_command_data_size)
                                    {
                                        ...
                                        case IPC::Command::CMD_DEBUG_MESSAGE:
                                            DebugLog()
                                                << *reinterpret_cast<const IPC::DebugMessage*>(
                                                       buf.data());
                                            break;
                                        ...
                                        case IPC::Command::CMD_COMMAND_BUF:
                                            ReceiveCommandSlotPointer(
                                                *reinterpret_cast<LPVOID*>(buf.data()));
                                            break;
                                        case IPC::Command::CMD_INPUT_BUF:
                                            ReceiveInputsPointer(
                                                *reinterpret_cast<LPVOID*>(buf.data()));
                                            break;
                                        case IPC::Command::CMD_DLL_LOAD_INFO_BUF:
                                            ReceiveDllLoadInfosPointer(
                                                *reinterpret_cast<LPVOID*>(buf.data()));
                                            break;
                                        case IPC::Command::CMD_TAS_FLAGS_BUF:
                                            ReceiveTASFlagsPointer(
                                                *reinterpret_cast<LPVOID*>(buf.data()));
                                            break;
                                        case IPC::Command::CMD_SOUND_INFO:
                                            ReceiveSoundCaptureInfoPointer(
                                                *reinterpret_cast<LPVOID*>(buf.data()));
                                            break;
                                        case IPC::Command::CMD_GENERAL_INFO:
                                            ReceiveGeneralInfoPointer(
                                                *reinterpret_cast<LPVOID*>(buf.data()));
                                            break;
                                        case IPC::Command::CMD_PALETTE_ENTRIES:
                                            ReceivePaletteEntriesPointer(
                                                *reinterpret_cast<LPVOID*>(buf.data()));
                                            break;
                                        ...
                                        case IPC::Command::CMD_KEYBOARD_LAYOUT_NAME:
                                            ReceiveKeyboardLayout(*reinterpret_cast<LPVOID*>(
                                                                      buf.data()),
                                                                  hGameProcess);
                                            break;
                                        case IPC::Command::CMD_SUGGEST_THREAD_NAME:
                                            SuggestThreadName(
                                                de.dwThreadId,
                                                de.dwProcessId,
                                                *reinterpret_cast<IPC::SuggestThreadName*>(
                                                    buf.data()));
                                            {
                                                DWORD bytes_written;
                                                WriteProcessMemory(hGameProcess,
                                                                   const_cast<LPVOID>(
                                                                       ipc_frame.m_command_data),
                                                                   buf.data(),
                                                                   ipc_frame.m_command_data_size,
                                                                   &bytes_written);
                                                // print the callstack of the thread
                                                std::map<DWORD, ThreadInfo>::iterator found =
                                                    hGameThreads.find(de.dwThreadId);
                                                if (found != hGameThreads.end())
                                                {
                                                    DebugLog()
                                                        << "Tracing thread (id=" << std::hex
                                                        << found->first
                                                        << ") (name=" << found->second.name << ")";
                                                    DbgHelp::StackWalk(de.dwProcessId,
                                                                       found->second.handle,
                                                                       PrintStackTrace);
                                                }
                                            }
                                            ...
                                            if (found != hGameThreads.end())
                                            {
                                                ...
                                                DbgHelp::StackWalk(
                                                    ...
                                                    [&de,
                                                     ...
                                                     &module_depth,
                                                     &current_module](
                                                        IDbgHelpStackWalkCallback& data) {
                                                        if (current_module == data.GetModuleName())
                                                        {
                                                            return IDbgHelpStackWalkCallback::
                                                                Action::CONTINUE;
                                                        }
                                                        is_trusted = TrustedModule::IsTrusted(
                                                            de.dwProcessId, data.GetModuleName());
                                                        ...
                                                        return IDbgHelpStackWalkCallback::Action::
                                                            CONTINUE;
                                                    });
                                                reinterpret_cast<IPC::TrustedCaller*>(buf.data())
                                                    ->SetTrusted(is_trusted);
                                            }
                                        }
                                        break;
                                        case IPC::Command::CMD_SAVE_STATE:
                                            SaveGameStatePhase2(
                                                *reinterpret_cast<int*>(buf.data()));
                                            break;
                                        ...
                                        {
                                            IPC::AutoWatch* auto_watch =
                                                reinterpret_cast<IPC::AutoWatch*>(buf.data());
                                        ...
                                        {
                                            FrameBoundary(
                                                *reinterpret_cast<IPC::FrameBoundaryInfo*>(
                                                    buf.data()),
                                                de.dwThreadId);
                                            ...
                                            if (s_lastFrameCount <= 1 && !requestedCommandReenter)
                                            {
                                                ...
                                                for (int i = 0; i < numThreads; i++)
                                                {
                                                    ...
                                                    ThreadInfo& info = hGameThreads[threadId];
                                                    HANDLE hThread = info.handle;
                                                    DebugLog() << "Tracing thread (id=" << std::hex
                                                               << threadId
                                                               << ") (name=" << info.name << ")";
                                                    DbgHelp::StackWalk(de.dwProcessId,
                                                                       hThread,
                                     ...
                                     * -- Warepire
                                     */
                                        ipc_frame.m_command = static_cast<IPC::Command>(
                                            static_cast<std::underlying_type<IPC::Command>::type>(
                                                ipc_frame.m_command)
                                            + 1);
                                        DWORD bytes_written;
                    ...
                    //else
                    {
                        if (de.u.Exception.ExceptionRecord.ExceptionCode != THREAD_NAME_EXCEPTION)
                        {
                            bool couldBeSerious =
                                ...
                                || ((de.u.Exception.ExceptionRecord.ExceptionCode & 0xCFFFFFFF)
                                        != /*EXCEPTION_MSVC*/ 0xC06D7363
                                    && (de.u.Exception.ExceptionRecord.ExceptionCode & 0xCFFFFFFF)
                                           != 0xC0440001);
...
#endif
                            {
                                    ...
                                    == EXCEPTION_ACCESS_VIOLATION) // 0xC0000005
                                {
                                    ...
                                    DebugLog() << "exception " << std::hex << r.ExceptionCode
                                               << " (at " << r.ExceptionAddress << "): attempt to "
                                               << (r.ExceptionInformation[0] == 0
                                                       ? "read"
                                ...
                                else if (couldBeSerious)
                                {
                                    EXCEPTION_RECORD& r = de.u.Exception.ExceptionRecord;
                                    DebugLog()
                                        << "exception " << std::hex << r.ExceptionCode << " (at "
                                        << r.ExceptionAddress
                                ...
                                if (couldBeSerious)
                                {
                                    ...
                                    {
                                        DebugLog()
                                            << "Tracing thread (id=" << std::hex << found->first
                                            << ") (name=" << found->second.name << ")";
                                        DbgHelp::StackWalk(de.dwProcessId,
                                                           found->second.handle,
                                                           PrintStackTrace);
                                ...
                                if (couldBeSerious && !de.u.Exception.dwFirstChance)
                                {
                                    ...
                                    for (int i = 0; i < numThreads; i++)
                                    {
                                        ...
                                        if (threadId == de.dwThreadId)
                                            continue;
                                        ThreadInfo& info = hGameThreads[threadId];
                                        DebugLog() << "Tracing thread (id=" << std::hex << threadId
                                                   << ") (name=" << info.name << ")";
                                        DbgHelp::StackWalk(de.dwProcessId,
                                                           info.handle,
                                                           PrintStackTrace);
                        ...
                        else if (de.u.Exception.ExceptionRecord.ExceptionCode
                                 == THREAD_NAME_EXCEPTION)
                        {
                            ...
                            THREADNAME_INFO* nameInfo =
                                (THREADNAME_INFO*)
                                    de.u.Exception.ExceptionRecord.ExceptionInformation;
                            if (nameInfo && nameInfo->dwType == 0x1000)
                            {
                                DWORD threadId = nameInfo->dwThreadID;
                                if (threadId == -1)
                                    threadId = de.dwThreadId;
                                std::map<DWORD, ThreadInfo>::iterator threadInfoIter =
                                    hGameThreads.find(threadId);
                                if (threadInfoIter != hGameThreads.end())
                                {
                                    ...
                                    SIZE_T bytesRead = 0;
                                    ReadProcessMemory(/*threadInfo.hProcess*/ hGameProcess,
                                                      (void*) nameInfo->szName,
                                                      threadInfo.name,
                                                      64,
                                                      &bytesRead);
                                    ...
                                    if (*oldName)
                                        DebugLog() << "got name for thread id=" << std::hex
                                ...
                                else
                                {
                                    ...
                                    threadInfo.name[63] = '\0';
                                    DebugLog()
                                        << "got name for as-yet-invalid thread id=" << std::hex
                                        << threadId << ": \"" << threadInfo.name << "\"";
                        ...
                        else if (de.u.Exception.dwFirstChance
                                 ...
                                 == EXCEPTION_INT_DIVIDE_BY_ZERO) // 0xC0000094
                        {
                            ...
                            if (found != hGameThreads.end())
                            {
                                if (GetThreadContext(found->second.handle, &context))
                                {
                                    char bytes[32] = {0};
                                    SIZE_T bytesRead = 0;
                                    if (ReadProcessMemory(/*(found->second).hProcess*/ hGameProcess,
                                                          (void*) context.Eip,
                                                          bytes,
                                                          32,
                                                          &bytesRead))
                                    {
                                        int divInsLen =
                                            instructionLength((const unsigned char*) bytes);
                        ...
                        else // actual crash:
                        {
...

                            {
                                ...
                                static LPCWSTR waitskipmsg =
                                    L"If this happens when fast-forwarding, try disabling \"Wait "
                                    L"Skip\" in \"Misc > Fast-Forward Options\".\n";
                                static LPCWSTR multidisablemsg =
                                    L"If this happens sometimes randomly, try switching "
                                    L"Multithreading to \"Disable\" in \"Misc > Multithreading "
                                    L"Mode\".\n";
                                static LPCWSTR dsounddisablemsg =
                                    L"Or choose \"Sound > Disable DirectSound Creation\" if the "
                                    L"game doesn't work without Multithreading on.\n";
                                static LPCWSTR dsounddisablemsg2 =
                                    L"If this happens sometimes randomly, try choosing \"Sound > "
                                ...
                                swprintf(msg,
                                         ...
                                         (localTASflags.fastForward
                                          && (localTASflags.fastForwardFlags & FFMODE_WAITSKIP))
                                             ? waitskipmsg
                                             : L"",
                                         (s_lastFrameCount > 30 && localTASflags.threadMode != 0
...
#pragma region LOAD_DLL_DEBUG_EVENT
                        ...
                        && de.u.LoadDll.hFile != nullptr)
                    {
                        ...
                        DbgHelp::LoadSymbols(de.dwProcessId,
                                             de.u.LoadDll.hFile,
                                             filename.c_str(),
                                             reinterpret_cast<DWORD64>(de.u.LoadDll.lpBaseOfDll));
...
#pragma region CREATE_THREAD_DEBUG_EVENT
                    DebugLog() << "STARTED THREAD: id=" << std::hex << de.dwThreadId
                               << ", handle=" << de.u.CreateThread.hThread;
                    ...
                    for (iter = hGameThreads.begin(); iter != hGameThreads.end(); iter++)
                    {
                        ...
                        DebugLog() << "thread status: id=" << std::hex << threadId
                                   << ", handle=" << hThread << ", suspend=" << std::dec
                                   << suspendCount << ", name=" << threadInfo.name;
                        DebugLog() << "Tracing thread (id=" << std::hex << threadId
...
#pragma region CREATE_PROCESS_DEBUG_EVENT
                    std::wstring filename =
                        GetFileNameFromProcessHandle(de.u.CreateProcessInfo.hProcess);
                    ...
                    else
                    {
                        ...
                        //						CloseHandle(hGameProcess);
                        hGameProcess = processInfo.hProcess;
                        DebugLog() << "switched to child process, handle " << std::hex
                                   << hGameProcess << ", PID = " << processInfo.dwProcessId << ".";
                        PrintPrivileges(hGameProcess);
                        DebugLog() << "attempting injection...";
                        InjectDll(processInfo.hProcess,
                                  processInfo.dwProcessId,
...
#pragma endregion
                ...
                case RIP_EVENT: // I've never encountered a rip event, but I might as well log it in case it happens.
                    DebugLog() << "RIP: err=" << std::hex << de.u.RipInfo.dwError
                               << ", type=" << de.u.RipInfo.dwType;
                    ;
                    break;
                }
...
void OnAfterDebugThreadExit()
{
    ...
    hAfterDebugThreadExitThread =
        Utils::Thread::CreateThread(NULL, 0, AfterDebugThreadExitThread, NULL, 0, NULL);
}
...
rewait:
    ...
    if (debuggerThread)
    {
        int result = CustomMessageBox(L"The game-controlling thread is taking longer than expected "
                                      L"to exit.\nClick Cancel to terminate it immediately (this "
                                      L"might cause problems),\nor click Retry to check again and "
                                      L"wait a few more seconds if needed.",
                                      L"Stopping Game",
...
rewait:
    ...
    {
        int result =
            CustomMessageBox(L"A thread is taking longer than expected to exit.\nClick Cancel to "
                             L"terminate it immediately (this might cause problems),\nor click "
                             L"Retry to check again and wait a few more seconds if needed.",
                             L"Stopping",
                             MB_RETRYCANCEL | MB_DEFBUTTON1 | MB_ICONWARNING);
        if (result == IDRETRY)
        {
...
void PrepareForExit()
{
    ...
    if (localTASflags
            .aviMode) // hack, sometimes ~AviFrameQueue has trouble terminating the thread, no sense waiting for it if there's no avi and we're exiting anyway
    {
        CloseAVI();
    }
    if (unsavedMovieData)
...
int APIENTRY wWinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPTSTR lpCmdLine, int nCmdShow)
{
    ...
    while (true)
    {
        ...
        if (!started /* && GetActiveWindow() == hWnd*/)
        {
            ...
            CheckHotkeys(
                -1,
                /*false*/ true); // let's count "before any frame" as framesynced as far as hotkeys
        }
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
    ...
    SendMessageW(hWnd,
                 ...
                 WPARAM(ICON_SMALL),
                 LPARAM(LoadImageW(
                     hInst, MAKEINTRESOURCEW(IDI_MYICON), IMAGE_ICON, 16, 16, LR_DEFAULTCOLOR)));
    SendMessageW(hWnd,
                 WM_SETICON,
                 WPARAM(ICON_BIG),
                 LPARAM(LoadImageW(
                     hInst, MAKEINTRESOURCEW(IDI_MYICON), IMAGE_ICON, 32, 32, LR_DEFAULTCOLOR)));
...
static void EnableDisablePlayRecordButtons(HWND hDlg)
{
    ...
    if (exeFileExists != exeexists || movieFileExists != exists || movieFileWritable != writable)
    {
...
void BringGameWindowToForeground()
{
    ...
    for (iter = gameHWnds.begin(); iter != gameHWnds.end();)
    {
        ...
        iter++;
        if ((style & (WS_VISIBLE | WS_CAPTION)) == (WS_VISIBLE | WS_CAPTION)
            || iter == gameHWnds.end())
        {
            RECT rect;
            GetClientRect(gamehwnd, &rect);
...
void SplitToValidPath(const char* initialPath,
...
void SplitToValidPath(LPCWSTR initialPath, LPCWSTR defaultDir, LPWSTR filename, LPWSTR directory)
{
    ...
    if (!wcsncmp(filename, directory, wcslen(directory)))
        memmove(filename,
                filename + wcslen(directory),
                (1 + wcslen(filename) - wcslen(directory)) * 2);
    ...
    case WM_COMMAND:
    {
        ...
        case ID_RAM_SEARCH:
            if (!RamSearchHWnd)
            {
                ...
                RamSearchHWnd = CreateDialog(hInst,
                                             MAKEINTRESOURCE(IDD_RAMSEARCH),
                                             hWnd,
                                             (DLGPROC) RamSearchProc);
            }
            else
        ...
        case ID_RAM_WATCH:
            if (!RamWatchHWnd)
            {
                ...
                RamWatchHWnd = CreateDialog(hInst,
                                            MAKEINTRESOURCE(IDD_RAMWATCH),
                                            hWnd,
                                            (DLGPROC) RamWatchProc);
            }
            else
        ...
        case ID_SOUND_RATE_8000:
            localTASflags.audioFrequency = 8000;
            tasFlagsDirty = true;
            break;
        case ID_SOUND_RATE_11025:
            localTASflags.audioFrequency = 11025;
            tasFlagsDirty = true;
            break;
        case ID_SOUND_RATE_12000:
            localTASflags.audioFrequency = 12000;
            tasFlagsDirty = true;
            break;
        case ID_SOUND_RATE_16000:
            localTASflags.audioFrequency = 16000;
            tasFlagsDirty = true;
            break;
        case ID_SOUND_RATE_22050:
            localTASflags.audioFrequency = 22050;
            tasFlagsDirty = true;
            break;
        case ID_SOUND_RATE_24000:
            localTASflags.audioFrequency = 24000;
            tasFlagsDirty = true;
            break;
        case ID_SOUND_RATE_32000:
            localTASflags.audioFrequency = 32000;
            tasFlagsDirty = true;
            break;
        case ID_SOUND_RATE_44100:
            localTASflags.audioFrequency = 44100;
            tasFlagsDirty = true;
            break;
        case ID_SOUND_RATE_48000:
            localTASflags.audioFrequency = 48000;
            tasFlagsDirty = true;
            break;
        case ID_SOUND_BITS_8:
            localTASflags.audioBitsPerSecond = 8;
            tasFlagsDirty = true;
            break;
        case ID_SOUND_BITS_16:
            localTASflags.audioBitsPerSecond = 16;
            tasFlagsDirty = true;
            break;
        case ID_SOUND_CHANNELS_1:
            localTASflags.audioChannels = 1;
            tasFlagsDirty = true;
            break;
        case ID_SOUND_CHANNELS_2:
            localTASflags.audioChannels = 2;
            tasFlagsDirty = true;
            break;
        ...
        case IDC_MAKETOPMOST:
            localTASflags.windowActivateFlags =
                (localTASflags.windowActivateFlags & ~2)
                | (IsDlgButtonChecked(hDlg, IDC_MAKETOPMOST) ? 2 : 0);
            tasFlagsDirty = true;
            break;
        ...
        case ID_TIME_RATE_SLOWER:
            if (localTASflags.timescale == localTASflags.timescaleDivisor)
            {
                localTASflags.timescale = 3;
                localTASflags.timescaleDivisor = 4;
                tasFlagsDirty = true;
            }
            else if (localTASflags.timescale * 4 == localTASflags.timescaleDivisor * 3)
            {
                localTASflags.timescale = 1;
                localTASflags.timescaleDivisor = 2;
                tasFlagsDirty = true;
            }
            else if (localTASflags.timescale * 2 == localTASflags.timescaleDivisor)
            {
                localTASflags.timescale = 1;
                localTASflags.timescaleDivisor = 4;
                tasFlagsDirty = true;
            }
            else if (localTASflags.timescale * 4 == localTASflags.timescaleDivisor)
            {
                localTASflags.timescale = 1;
                localTASflags.timescaleDivisor = 8;
                tasFlagsDirty = true;
        ...
        case ID_TIME_RATE_FASTER:
            if (localTASflags.timescale * 4 == localTASflags.timescaleDivisor * 3)
            {
                localTASflags.timescale = 1;
                localTASflags.timescaleDivisor = 1;
                tasFlagsDirty = true;
            ...
            else if (localTASflags.timescale * 8 == localTASflags.timescaleDivisor)
            {
                localTASflags.timescale = 1;
                localTASflags.timescaleDivisor = 4;
                tasFlagsDirty = true;
        ...
        case ID_EXEC_LOCALE_SYSTEM:
            localTASflags.appLocale = 0;
            tasFlagsDirty = true;
            break;
        case ID_EXEC_LOCALE_JAPANESE:
            localTASflags.appLocale = 1041;
            tasFlagsDirty = true;
            break;
        case ID_EXEC_LOCALE_CHINESE:
            localTASflags.appLocale = 2052;
            tasFlagsDirty = true;
            break;
        case ID_EXEC_LOCALE_KOREAN:
            localTASflags.appLocale = 1042;
            tasFlagsDirty = true;
            break;
        case ID_EXEC_LOCALE_ENGLISH:
            localTASflags.appLocale = 1033;
            tasFlagsDirty = true;
            break;
        ...
        case ID_INPUT_INPUTS:
        {
            if (HotkeyHWnd == NULL)
            {
                ...
                HotkeyHWnd = CreateDialog(hInst,
                                          MAKEINTRESOURCE(IDD_CONTROLCONF),
                                          hWnd,
                                          (DLGPROC) &InputCapture::ConfigureInput);
                ...
                if (dialogPosY < desktopRect.top)
                    dialogPosY = desktopRect.top;
                if (dialogPosX < desktopRect.left)
                    dialogPosX = desktopRect.left;
                if (dialogPosY + static_cast<int>(dialogSizeY) > desktopRect.bottom)
                    dialogPosY = desktopRect.bottom - dialogSizeY;
                if (dialogPosX + static_cast<int>(dialogSizeX) > desktopRect.right)
                    dialogPosX = desktopRect.right - dialogSizeX;

                SetWindowPos(HotkeyHWnd,
                             HWND_TOP,
                             dialogPosX,
                             dialogPosY,
                             dialogSizeX,
        ...
        case ID_INCLUDE_LCF_NONE:
            ...
            localTASflags.log_categories[static_cast<std::underlying_type<LogCategory>::type>(
                LogCategory::ANY)] = true;
            tasFlagsDirty = true;
            break;
        ...
        case ID_PERFORMANCE_TOGGLESAVEVIDMEM:
            localTASflags.storeVideoMemoryInSavestates =
                !localTASflags.storeVideoMemoryInSavestates;
            tasFlagsDirty = true;
            break;
        case ID_PERFORMANCE_TOGGLESAVEGUARDED:
            storeGuardedPagesInSavestates = !storeGuardedPagesInSavestates;
            mainMenuNeedsRebuilding = true;
            break;
        ...
        case ID_AVI_VIDEO:
            ...
            if (!CreateAVIFile())
            {
                ...
                CustomMessageBox(L"The file cannot be created here.\nProbable causes are that the "
                                 L"location is write protected\nor the file name exceeds the "
                                 L"maximum allowed characters by Windows (260 characters, with "
                                 L"extension)\n",
            ...
            if (HIWORD(wParam) == EN_CHANGE)
            {
                ...
                if (started && !warnedAboutFrameRateChange)
                {
                    ...
                    if (localTASflags.framerate != wcstol(str, 0, 0))
                    {
                        ...
                        int result = CustomMessageBox(
                            L"If you are in the middle of recording or playing a movie,\nchanging "
                            L"the game's framerate might cause desync in some games.\nBe careful.",
                            L"Warning",
                        ...
                        if (result == IDCANCEL)
                        {
                            ...
                            warnedAboutFrameRateChange =
                                false; // must be true above this to avoid infinite warnings
                            break;
                        }
                    }
                }
        ...
        case IDC_BUTTON_RECORD:
            if (IsWindowEnabled(GetDlgItem(hDlg, IDC_BUTTON_RECORD)))
            {
                ...
                EnableWindow(GetDlgItem(hDlg, IDC_BUTTON_STOP), false);
                {
                    ...
                    GetWindowTextW(GetDlgItem(hWnd, IDC_EDIT_COMMANDLINE),
                                   commandline,
                                   ARRAYSIZE(commandline));
                    command_line = commandline;
                }
                debuggerThread =
                    Utils::Thread::CreateThread(NULL, 0, DebuggerThreadFunc, NULL, 0, NULL);
            }
            break;
        ...
        case IDC_BUTTON_GAMEBROWSE:
        {
            std::wstring exe_file_name =
                Utils::File::GetFileNameOpen(this_process_path,
        ...
        case IDC_BUTTON_MOVIEBROWSE:
            if (!started)
            {
                ...
                bool isSave =
                    (command == ID_FILES_RECORDMOV /* || command == ID_FILES_RESUMEMOVAS*/);
                ...
                {
                    movie_file_name =
                        Utils::File::GetFileNameSave(this_process_path,
                                                     {Utils::File::FileFilter::HourglassMovie,
                                                      Utils::File::FileFilter::AllFiles});
                }
        ...
        case ID_FILES_BACKUPMOVIE:
        {
            ...
            std::wstring movie_file_name =
                Utils::File::GetFileNameSave(movie_filename,
                                             {Utils::File::FileFilter::HourglassMovie,
                                              Utils::File::FileFilter::AllFiles});
        ...
        case IDC_TEXT_EXE:
            if (HIWORD(wParam) == EN_CHANGE || !movieFileExists || movie_filename.empty())
            {
                ...
                if (!exeFileExists) // Extra check, do we need it?
                {
                    ...
                    swprintf(str,
                             ARRAYSIZE(str),
                             L"The exe file \'%s\' does not exist, please check that it has not "
                             L"been renamed or moved.",
                             exe_filename.c_str());
                ...
                {
                    DebugLog() << "Detecting the default stack size succeeded, size is: "
                               << localTASflags.threadStackSize << " bytes.";
                }
                else
                {
                    WCHAR str[1024];
                    swprintf(str,
                             ARRAYSIZE(str),
                             L"Determinating the default thread stack size failed!\nVerify that "
                             L"'%s' a valid Win32 executable.",
                             exe_filename.c_str());
                    CustomMessageBox(str, L"Error!", (MB_OK | MB_ICONERROR));
                    recursing = false;
                    break;
                ...
                //{
                //	fclose(file);
                if ((!movienameCustomized && HIWORD(wParam) == EN_CHANGE) || !movieFileExists
                    || movie_filename.empty())
                {
                    ...
                    if (newLagskip != advancePastNonVideoFrames
                        && (!advancePastNonVideoFramesConfigured || !advancePastNonVideoFrames))
                    {
                ...
                if (!movieFileExists || movie_filename.empty()
                    || !movienameCustomized) // set default movie name based on exe
                {
                    bool setMovieFile = true;
                    if (movieFileExists)
                    {
                        std::wstring exefname = GetExeFilenameWithoutPath();
                        if (!wcsncmp(exefname.c_str(),
                                     movie_filename.c_str(),
                                     exefname.length()
                                         - 4)) // Compare names, without the file extension.
        ...
        case IDC_TEXT_MOVIE: // Whenever IDC_TEXT_MOVIE changes, a new movie has been chosen for whatever reason.
            if (HIWORD(wParam) == EN_CHANGE)
            {
                ...
                if (tmp_movie.empty())
                    break; // No movie loaded, don't go forward with anything.
                ...
                {
                    int result = CustomMessageBox(L"The currently opened movie contains unsaved "
                                                  L"data.\nDo you want to save it before opening "
                                                  L"the new movie?\n(Click \"Yes\" to save, \"No\" "
                                                  L"to proceed without saving)",
                                                  L"Warning!",
                    ...
                    unsavedMovieData = false;
                    movie.headerBuilt =
                        false; // Otherwise we may retain the old header? // TODO: Find this out
                }

                if (!movie_filename
                         .empty()) // We have currently locked the directory that we're saving the old movie to.
                {
                ...
                {
                    movie_filename
                        .clear(); // Let's not keep the old movie file name, or there could be some non-desired overwriting taking place.
