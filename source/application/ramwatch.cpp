...
bool RWfileChanged =
    false; //Keeps track of whether the current watch file has been changed, if so, ramwatch will prompt to save changes
...
bool IsSameWatch(const AddressWatcher& l, const AddressWatcher& r)
{
    return ((l.Address == r.Address) && (l.Size == r.Size)
            && (l.Type == r.Type) /* && (l.WrongEndian == r.WrongEndian)*/);
}
...
LRESULT CALLBACK PromptWatchNameProc(HWND hDlg,
                                     UINT uMsg,
                                     WPARAM wParam,
                                     LPARAM lParam) //Gets the description of a watched address
{
    RECT r;
    RECT r2;
    int dx1, dy1, dx2, dy2;
...
void Update_RAM_Watch()
{
    ...
    int bottom = top + ListView_GetCountPerPage(lv)
                 + 1; // +1 is so we will update a partially-displayed last item
    if (top < 0)
        top = 0;
    if (bottom > WatchCount)
        bottom = WatchCount;
    int start = -1;
    ...
    for (x = MAX_RECENT_WATCHES - 1; x >= 0; x--)
    {
        ...
        else
        {
            swprintf(tmp,
                     ...
                     L"&%d. %s",
                     (x + 1) % 10,
                     rw_recent_files[x] + wcslen(rw_recent_files[x]) - 127);
        }
...
void UpdateRWRecentArray(LPCWSTR addString,
...
{
    UpdateRWRecentArray(filename,
                        MAX_RECENT_WATCHES,
                        rwrecentmenu,
                        RAMMENU_FILE_RECENT,
                        RW_MENU_FIRST_RECENT_FILE);
}
...
bool Save_Watches()
{
    ...
    wcscpy(Str_Tmp_RW,
           Config::exe_filename.substr((slash != std::wstring::npos) ? slash + 1 : 0).c_str());
    ...
    if (Change_File_S(Str_Tmp_RW))
    {
        ...
        if (!WatchFile)
            WatchFile = _wfopen(Str_Tmp_RW, L"w+b");
        ...
        {
            swprintf(Str_Tmp_RW,
                     ARRAYSIZE(Str_Tmp_RW),
                     L"%05X%c%08X%c%c%c%c%c%d%c%s\n",
                     i,
                     DELIM,
                     rswatches[i].Address,
                     DELIM,
...
bool QuickSaveWatches()
{
    if (RWfileChanged == false)
        return true; //If file has not changed, no need to save changes
    if (currentWatch[0]
        == NULL) //If there is no currently loaded file, run to Save as and then return
    {
        return Save_Watches();
    }
...
bool ReplaceWatch(const AddressWatcher& Watch, LPWSTR Comment, int index)
{
    ...
    if (InsertWatch(Watch, Comment))
    {
        ...
        if (moveCount > 0)
        {
            ...
            memmove(&(rswatches[index + 1]),
                    &(rswatches[index]),
                    sizeof(AddressWatcher) * moveCount);
...
LRESULT CALLBACK
EditWatchProc(HWND hDlg,
              UINT uMsg,
              WPARAM wParam,
              LPARAM lParam) //Gets info for a RAM Watch, and then inserts it into the Watch List
{
    RECT r;
    RECT r2;
    int dx1, dy1, dx2, dy2;
    ...
    case WM_COMMAND:
        ...
        case IDOK:
        {
            if (s && t)
            {
                ...
                if (wcslen(Str_Tmp_RW) > 8)
                    addrstr = &(Str_Tmp_RW[wcslen(Str_Tmp_RW) - 9]);
                ...
                if (!VerifyWatchNotAlreadyAdded(Temp, index))
                {
                    int result = MessageBox(hDlg,
                                            L"Watch already exists. Replace it?",
                                            L"REPLACE",
                                            MB_YESNO);
                    if (result == IDYES)
                ...
                if (!canceled)
                {
                    if (IsHardwareAddressValid(Temp.Address) || !Config::started
                        || (index < WatchCount && Temp.Address == rswatches[index].Address))
                    {
                        GetDlgItemText(hDlg, IDC_PROMPT_EDIT, Str_Tmp_RW, 80);
                        ReplaceWatch(Temp, Str_Tmp_RW, index);
                        if (RamWatchHWnd)
                        {
                            ListView_SetItemCount(GetDlgItem(RamWatchHWnd, IDC_WATCHLIST),
                                                  WatchCount);
                        }
                        EndDialog(hDlg, true);
                    }
...
LRESULT CALLBACK RamWatchProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITMENU:
        ...
        CheckMenuItem(ramwatchmenu,
                      RAMMENU_FILE_SAVEWINDOW,
                      RWSaveWindowPos ? MF_CHECKED : MF_UNCHECKED);
        break;
    ...
    case WM_COMMAND:
        ...
        case IDC_C_WATCH_EDIT:
            ...
            DialogBoxParam(hInst,
                           MAKEINTRESOURCE(IDD_EDITWATCH),
                           hDlg,
                           (DLGPROC) EditWatchProc,
                           (LPARAM) watchIndex);
            SetFocus(GetDlgItem(hDlg, IDC_WATCHLIST));
            return true;
        case IDC_C_WATCH:
        ...
        case IDC_C_WATCH_UP:
        {
            ...
            ListView_SetItemState(GetDlgItem(hDlg, IDC_WATCHLIST),
                                  watchIndex - 1,
        ...
        case RAMMENU_FILE_AUTOLOAD:
        {
            AutoRWLoad ^= 1;
            CheckMenuItem(ramwatchmenu,
                          RAMMENU_FILE_AUTOLOAD,
                          AutoRWLoad ? MF_CHECKED : MF_UNCHECKED);
            break;
        }
        ...
        case IDCANCEL:
            ...
            return true;
        default:
            if (LOWORD(wParam) >= RW_MENU_FIRST_RECENT_FILE
                && LOWORD(wParam) < RW_MENU_FIRST_RECENT_FILE + MAX_RECENT_WATCHES)
