...
struct RSVal
{
    ...
    {
        if (v.i32s.i1 != r.v.i32s.i1)
            return false;
        if (t != t_ll && t != t_d && r.t != t_ll && r.t != t_d)
            return true;
...
#define ALIGN16       \
    __declspec(align( \
        16)) // 16-byte alignment speeds up memcpy for size >= 0x100 (as of VS2005, if SSE2 is supported at runtime)
#else
