...
void HelperFuncInsertMenuByID(HMENU smenu, UINT pos, UINT flags, UINT_PTR id, LPCWSTR suffix, LPCWSTR str, LPCWSTR disableReason)
{
	WCHAR Str_Tmp[1024];
#if 0
...
__forceinline void HelperFuncInsertMenu(HMENU smenu, UINT pos, UINT flags, HMENU menu, LPCWSTR suffix, LPCWSTR str, LPCWSTR disableReason)
{
...
void Build_Main_Menu(HMENU& MainMenu, HWND hWnd)
{
	...
	HelperFuncInsertMenuByID(Files, i++, Flags | ((started || !exeFileExists) ? MF_GRAYED : 0), ID_FILES_RECORDMOV, L"\tCtrl+R", L"&Record New Movie...", started ? L"must stop running first" : L"must open an executable first");
	HelperFuncInsertMenuByID(Files, i++, Flags | ((!started || localTASflags.playback) ? MF_GRAYED : 0), ID_FILES_BACKUPMOVIE, L"", L"Backup Movie to File...", L"Must be recording");
#if 0
...
#endif
	HelperFuncInsertMenuByID(Files, i++, Flags | ((!started || !localTASflags.playback || finished) ? MF_GRAYED : 0), ID_FILES_RESUMERECORDING, L"", L"Resume Recording from Now", L"movie must be playing");
#if 0
