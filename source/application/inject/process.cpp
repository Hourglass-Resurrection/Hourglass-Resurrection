...
Process::Process(DWORD processID, HANDLE hProcess)
    : hProcess_(hProcess), processID_(processID), pebAddr_(0)
{
...
{
    LPVOID addr =
        VirtualAllocEx(hProcess_, desiredAddress, size, allocationType, PAGE_EXECUTE_READWRITE);
    if (addr == NULL)
...
uintptr_t Process::getImageBase(HANDLE hThread) const
{
    ...
    {
        throwSysError("Error while reading process memory to retrieve image base address",
                      GetLastError());
    }
    return iba;
}
