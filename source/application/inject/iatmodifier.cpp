...
void IATModifier::setImageBase(uintptr_t address)
{
    ...
    if (ntHeaders.FileHeader.Characteristics & IMAGE_FILE_DLL)
        throw std::runtime_error(
            "Error while setting image base address: not the image base of an executable");
    ...
    importDescrTblAddr_ = (PIMAGE_IMPORT_DESCRIPTOR)(
...
{
    if (dlls.empty())
        throw IATModifierException("DLL path list must not be empty");
    if (importDescrTblAddr_ == NULL)
        throw IATModifierException("Import descriptor must not be NULL");
    ...
    for (size_t i = 0; i < dlls.size(); ++i)
        customDataSize += padToDword(dlls[i].size() + 1);
    ...
    for (PIMAGE_IMPORT_DESCRIPTOR descrAddr = importDescrTblAddr_;; ++descrAddr)
    {
        ...
        if (tempIID.FirstThunk == 0 && tempIID.OriginalFirstThunk == 0)
            break;
    }
    ...
    DWORD newTblRVA = (DWORD) newDescrTblAddress - iba + origIIDTblSize
                      + dlls.size() * sizeof(IMAGE_IMPORT_DESCRIPTOR);
    ...
    PDWORD curBlock =
        (PDWORD)(newDescrTbl + origIIDTblSize + dlls.size() * sizeof(IMAGE_IMPORT_DESCRIPTOR));
    for (size_t i = 0; i < dlls.size(); ++i)
    {
    ...
    {
        uintptr_t sectionHdrAddr = ntHeadersAddr_ + FIELD_OFFSET(IMAGE_NT_HEADERS, OptionalHeader)
                                   + ntHeaders.FileHeader.SizeOfOptionalHeader;
        IMAGE_SECTION_HEADER ish;
        for (int i = 0; i < ntHeaders.FileHeader.NumberOfSections;
             ++i, sectionHdrAddr += sizeof(IMAGE_SECTION_HEADER))
        {
    ...
    if (ntHeaders.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BOUND_IMPORT].VirtualAddress
            != -1
    ...
    if (ntHeaders.OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_COM_DESCRIPTOR].VirtualAddress
        != 0)
    {
        uintptr_t comDescriptorAddr =
        ...
        if (cor20Header.Flags & COMIMAGE_FLAGS_ILONLY)
        {
            ...
            DWORD oldProtect = process_.protectMemory((LPVOID) comDescriptorAddr,
                                                      sizeof(IMAGE_COR20_HEADER),
                                                      PAGE_EXECUTE_READWRITE);
            process_.writeMemory((LPVOID) comDescriptorAddr,
                                 &cor20Header,
                                 sizeof(IMAGE_COR20_HEADER));
            process_.protectMemory((LPVOID) comDescriptorAddr,
                                   sizeof(IMAGE_COR20_HEADER),
                                   oldProtect);
        }
    }
    ...
    DWORD oldProtect = process_.protectMemory((void*) ntHeadersAddr_,
...
void* IATModifier::allocateMemAboveBase(void* baseAddress, size_t size)
{
    try
    {
        ...
        for (char* currentAddress = (char*) baseAddress;;
             currentAddress = (char*) mbi.BaseAddress + mbi.RegionSize)
        {
...
IMAGE_NT_HEADERS IATModifier::readNTHeaders() const
{
    if (ntHeadersAddr_ == 0)
        throw std::runtime_error(
            "Image base address has not been set - unable to retrieve IMAGE_NT_HEADERS");
    IMAGE_NT_HEADERS ntHeaders;
    process_.readMemory((void*) ntHeadersAddr_, &ntHeaders, sizeof(IMAGE_NT_HEADERS));
    return ntHeaders;
}
