...
{
    using item_type = impl::item_type<Enumerator>;
    ...
    void GetNextItem()
    {
        ...
        item_type* item;
        if (impl::ItemByIndex<Enumerator, item_type>(m_enumerator, m_next_index++, &item) == S_OK)
        {
            m_last_item = item;
            item->Release();
        }
...
public:
    DiaEnumIterator(CComPtr<Enumerator> enumerator, DWORD index)
        : m_enumerator(enumerator)
        , m_count(impl::GetCount<Enumerator>(enumerator))
        , m_next_index(index)
        , m_last_item(nullptr)
    {
        GetNextItem();
 ...
 */
template<class Enumerator>
DiaEnumIterator<Enumerator> begin(Enumerator* enumerator)
{
    return DiaEnumIterator<Enumerator>(enumerator, 0);
}

template<class Enumerator>
DiaEnumIterator<Enumerator> end(Enumerator* enumerator)
{
    return DiaEnumIterator<Enumerator>(enumerator, impl::GetCount(enumerator));
}

template<class Enumerator>
DiaEnumIterator<Enumerator> begin(CComPtr<Enumerator> enumerator)
{
    return DiaEnumIterator<Enumerator>(std::move(enumerator), 0);
}

template<class Enumerator>
DiaEnumIterator<Enumerator> end(CComPtr<Enumerator> enumerator)
{
    return DiaEnumIterator<Enumerator>(std::move(enumerator), impl::GetCount<Enumerator>(enumerator));
}
