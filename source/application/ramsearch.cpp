...
#pragma message(              \
    "ramsearch.inl(" LINELINE \
    ") : warning: your compiler is too old to compile this file. (VS2005 or newer has __VA_ARGS__ support)")
...
struct MemoryRegion
{
    ...
    unsigned int
        virtualIndex; // index into s_prevValues, s_curValues, and s_numChanges, valid after being initialized in ResetMemoryRegions()
...
static unsigned short* s_numChanges =
    0; // number of changes of the item starting at this virtual index address
static MemoryRegion** s_itemIndexToRegionPointer =
    0; // used for random access into the memory list (trading memory size to get speed here, too bad it's so much memory), only valid when s_itemIndicesInvalid is false
static BOOL s_itemIndicesInvalid =
    true; // if true, the link from listbox items to memory regions (s_itemIndexToRegionPointer) and the link from memory regions to list box items (MemoryRegion::itemIndex) both need to be recalculated
static BOOL s_prevValuesNeedUpdate =
    true; // if true, the "prev" values should be updated using the "cur" values on the next frame update signaled
static unsigned int s_maxItemIndex =
    0; // max currently valid item index, the listbox sometimes tries to update things past the end of the list so we need to know this to ignore those attempts
...
void ResetMemoryRegions()
{
    ...
    for (MemoryList::iterator iter = s_activeMemoryRegions.begin();
         iter != s_activeMemoryRegions.end();
         ++iter)
    {
    ...
    {
        s_prevValues =
            (unsigned char*) realloc(s_prevValues, sizeof(char) * (nextVirtualIndex + 8));
        ...
        s_numChanges =
            (unsigned short*) realloc(s_numChanges, sizeof(short) * (nextVirtualIndex + 8));
        memset(s_numChanges, 0, sizeof(short) * (nextVirtualIndex + 8));

        s_itemIndexToRegionPointer =
            (MemoryRegion**) realloc(s_itemIndexToRegionPointer,
                                     sizeof(MemoryRegion*) * (nextVirtualIndex + 8));
...
int DeactivateRegion(MemoryRegion& region,
                     MemoryList::iterator& iter,
                     HWAddressType hardwareAddress,
                     unsigned int size)
{
    if (hardwareAddress + size <= region.hardwareAddress
        || hardwareAddress >= region.hardwareAddress + region.size)
    {
        // region is unaffected
        return 0;
    }
    else if (hardwareAddress > region.hardwareAddress
             && hardwareAddress + size >= region.hardwareAddress + region.size)
    {
        // erase end of region
    ...
    else //if(hardwareAddress > region.hardwareAddress && hardwareAddress + size < region.hardwareAddress + region.size)
    {
        // split region
        int eraseSize = (hardwareAddress + size) - region.hardwareAddress;
        MemoryRegion region2 = {region.hardwareAddress + eraseSize,
                                region.size - eraseSize,
                                /*region.softwareAddress + eraseSize,*/ region.virtualIndex
                                    + eraseSize};
        region.size = hardwareAddress - region.hardwareAddress;
...
void CalculateItemIndices(int itemSize)
{
         ...
         ++iter)
    {
        ...
        int startSkipSize = ((unsigned int) (itemSize - (unsigned int) region.hardwareAddress))
                            % itemSize; // FIXME: is this still ok?
...
bool RSVal::print(LPWSTR output, size_t count, WCHAR sizeTypeID, WCHAR typeID)
{
    ...
    case 's':
        ...
        default:
        case 'b':
            output += swprintf(output, count, L"%d", (char) ((int) *this & 0xff));
            if ((unsigned int) (((int) *this & 0xff) - 32) < (unsigned int) (127 - 32))
                swprintf(output, count, L" ('%c')", (char) ((int) *this & 0xff));
            break;
        case 'w':
            swprintf(output, count, L"%d", (short) ((int) *this & 0xffff));
            break;
        case 'd':
            swprintf(output, count, L"%d", (int) *this);
            break;
        case 'l':
            swprintf(output, count, L"%I64d", (long long) *this);
            break;
        }
        break;
    case 'u':
        ...
        default:
        case 'b':
            output += swprintf(output, count, L"%u", (unsigned char) ((int) *this & 0xff));
            if ((unsigned int) (((int) *this & 0xff) - 32) < (unsigned int) (127 - 32))
                swprintf(output, count, L" ('%c')", (unsigned char) ((int) *this & 0xff));
            break;
        case 'w':
            swprintf(output, count, L"%u", (unsigned short) ((int) *this & 0xffff));
            break;
        case 'd':
            swprintf(output, count, L"%lu", (unsigned long) (int) *this);
            break;
        case 'l':
            swprintf(output, count, L"%I64u", (unsigned long long) (long long) *this);
            break;
        }
        break;
    default:
    case 'h':
        ...
        default:
        case 'b':
            swprintf(output, count, L"%02x", ((int) *this & 0xff));
            break;
        case 'w':
            swprintf(output, count, L"%04x", ((int) *this & 0xffff));
            break;
        case 'd':
            swprintf(output, count, L"%08x", (int) *this);
            break;
        case 'l':
            swprintf(output, count, L"%016I64x", (long long) *this);
            break;
        }
        break;
    }
...
void UpdateRegionT(const MemoryRegion& region, const MemoryRegion* nextRegionPtr)
{
    ...
    if (s_prevValuesNeedUpdate)
        memcpy(s_prevValues + region.virtualIndex,
               s_curValues + region.virtualIndex,
               region.size + sizeof(compareType) - sizeof(stepType));

    unsigned int startSkipSize =
        ((unsigned int) (sizeof(stepType) - region.hardwareAddress)) % sizeof(stepType);
    ...
    ReadProcessMemory(hGameProcess,
                      (const void*) region.hardwareAddress,
                      (void*) memoryBuffer,
                      region.size,
                      NULL);
    if (sizeof(compareType) > 1)
        ReadProcessMemory(hGameProcess,
                          (const void*) (region.hardwareAddress + region.size),
                          (void*) (memoryBuffer + region.size),
                          sizeof(compareType) - 1,
                          NULL);
    ...
    { // - more than one byte can affect a given change count entry
        ...
        unsigned int endSkipSize =
            ((unsigned int) (startSkipSize - region.size)) % sizeof(stepType);
        unsigned int lastIndexToRead =
            indexEnd + endSkipSize + sizeof(compareType) - sizeof(stepType);
        unsigned int lastIndexToCopy = lastIndexToRead;
        if (nextRegionPtr)
        {
            const MemoryRegion& nextRegion = *nextRegionPtr;
            int nextStartSkipSize =
                ((unsigned int) (sizeof(stepType) - nextRegion.hardwareAddress)) % sizeof(stepType);
        ...
        for (unsigned int i = indexStart, j = 0; i < lastIndexToRead; i++, j++)
        {
            if (s_curValues[i] != sourceAddr[i]) // if value of this byte changed
            {
                if (i < lastIndexToCopy)
                    s_curValues[i] = sourceAddr[i]; // update value
                for (int k = 0; k < sizeof(compareType);
                     k++) // loop through the previous entries that contain this byte
                {
                    if (i >= indexEnd + k)
                        continue;
                    int m = (j - k + sizeof(compareType)) & (sizeof(compareType) - 1);
                    if (nextValidChange[m]
                        <= i) // if we didn't already increase the change count for this entry
                    {
                        //if(s_numChanges[i-k] != 0xFFFF)
                        s_numChanges[i - k]++; // increase the change count for this entry
                        nextValidChange[m] =
                            i - k + sizeof(compareType); // and remember not to increase it again
                    }
                }
            }
        }
...
unsigned int HardwareAddressToItemIndex(HWAddressType hardwareAddress)
{
    ...
    {
        MemoryRegion& region = *iter;
        if (hardwareAddress >= region.hardwareAddress
            && hardwareAddress < region.hardwareAddress + region.size)
        {
...
#define CALL_WITH_T_SIZE_TYPES(functionName, sizeTypeID, typeID, requireAligned, ...)            \
    (typeID == 'f'                                                                               \
         ? (sizeTypeID == 'l' ? (requireAligned ? functionName<long long, double>(##__VA_ARGS__) \
                                                : functionName<char, double>(##__VA_ARGS__))     \
                              : (requireAligned ? functionName<long, float>(##__VA_ARGS__)       \
                                                : functionName<char, float>(##__VA_ARGS__)))     \
         : sizeTypeID == 'b'                                                                     \
               ? (typeID == 's' ? functionName<char, signed char>(##__VA_ARGS__)                 \
                                : functionName<char, unsigned char>(##__VA_ARGS__))              \
               : sizeTypeID == 'w'                                                               \
                     ? (typeID == 's'                                                            \
                            ? (requireAligned ? functionName<short, signed short>(##__VA_ARGS__) \
                     ...
                     : sizeTypeID == 'd'                                                         \
                           ...
                           : sizeTypeID == 'l'                                                   \
                                 ? (typeID == 's'                                                \
                                        ? (requireAligned                                        \
                                               ? functionName<long long, signed long long>(      \
                                                     ##__VA_ARGS__)                              \
                                               : functionName<char, signed long long>(           \
                                                     ##__VA_ARGS__))                             \
                                        : (requireAligned                                        \
                                               ? functionName<long long, unsigned long long>(    \
                                                     ##__VA_ARGS__)                              \
                                               : functionName<char, unsigned long long>(         \
                                                     ##__VA_ARGS__)))                            \
...
#define CALL_WITH_T_STEP(functionName, sizeTypeID, sign, type, requireAligned, ...)              \
    (sizeTypeID == 'b'                                                                           \
         ...
         : sizeTypeID == 'w'                                                                     \
               ? (requireAligned ? functionName<short, sign type>(##__VA_ARGS__)                 \
                                 : functionName<char, sign type>(##__VA_ARGS__))                 \
               : sizeTypeID == 'd'                                                               \
                     ? (requireAligned ? functionName<long, sign type>(##__VA_ARGS__)            \
                                       : functionName<char, sign type>(##__VA_ARGS__))           \
                     : sizeTypeID == 'l'                                                         \
                           ? (requireAligned ? functionName<long long, sign type>(##__VA_ARGS__) \
                                             : functionName<char, sign type>(##__VA_ARGS__))     \
                           : functionName<char, sign type>(##__VA_ARGS__))

// basic comparison functions:
template<typename T>
inline bool LessCmp(T x, T y, T i)
{
    return x < y;
}
template<typename T>
inline bool MoreCmp(T x, T y, T i)
{
    return x > y;
}
template<typename T>
inline bool LessEqualCmp(T x, T y, T i)
{
    return x <= y;
}
template<typename T>
inline bool MoreEqualCmp(T x, T y, T i)
{
    return x >= y;
}
template<typename T>
inline bool EqualCmp(T x, T y, T i)
{
    return x == y;
}
template<typename T>
inline bool UnequalCmp(T x, T y, T i)
{
    return x != y;
}
template<typename T>
inline bool DiffByCmp(T x, T y, T p)
{
    return x - y == p || y - x == p;
}
template<typename T>
inline bool ModIsCmp(T x, T y, T p)
{
    return p && x % p == y;
}
template<>
inline bool ModIsCmp(float x, float y, float p)
{
    return p && fmodf(x, p) == y;
}
template<>
inline bool ModIsCmp(double x, double y, double p)
{
    return p && fmod(x, p) == y;
}
...
void SearchRelative(bool (*cmpFun)(T, T, T), T ignored, T param)
{
         ...
         iter != s_activeMemoryRegions.end();)
    {
        ...
        for (unsigned int i = start, hwaddr = region.hardwareAddress; i < end;
             i += sizeof(stepType), hwaddr += sizeof(stepType))
            if (!cmpFun(GetCurValueFromVirtualIndex<stepType, T>(i),
                        GetPrevValueFromVirtualIndex<stepType, T>(i),
                        param))
...
#define DO_SEARCH(sf)                                      \
    ...
    {                                                      \
    case '<':                                              \
        DO_SEARCH_2(LessCmp, sf);                          \
        break;                                             \
    case '>':                                              \
        DO_SEARCH_2(MoreCmp, sf);                          \
        break;                                             \
    case '=':                                              \
        DO_SEARCH_2(EqualCmp, sf);                         \
        break;                                             \
    case '!':                                              \
        DO_SEARCH_2(UnequalCmp, sf);                       \
        break;                                             \
    case 'l':                                              \
        DO_SEARCH_2(LessEqualCmp, sf);                     \
        break;                                             \
    case 'm':                                              \
        DO_SEARCH_2(MoreEqualCmp, sf);                     \
        break;                                             \
    case 'd':                                              \
        DO_SEARCH_2(DiffByCmp, sf);                        \
        break;                                             \
    case '%':                                              \
        DO_SEARCH_2(ModIsCmp, sf);                         \
        break;                                             \
    default:                                               \
        assert(!"Invalid operator for this search type."); \
        break;                                             \
    }
...
#define DO_SEARCH_2(CmpFun, sf) \
    CALL_WITH_T_SIZE_TYPES(sf, rs_type_size, t, noMisalign, CmpFun, v, p)
    case 'r':
        DO_SEARCH(SearchRelative);
        break;
    case 's':
        DO_SEARCH(SearchSpecific);
        break;

#undef DO_SEARCH_2
#define DO_SEARCH_2(CmpFun, sf) \
    CALL_WITH_T_STEP(sf, rs_type_size, unsigned, int, noMisalign, CmpFun, v, p);
    case 'a':
        DO_SEARCH(SearchAddress);
        break;

#undef DO_SEARCH_2
#define DO_SEARCH_2(CmpFun, sf) \
    CALL_WITH_T_STEP(sf, rs_type_size, unsigned, short, noMisalign, CmpFun, v, p);
    case 'n':
        DO_SEARCH(SearchChanges);
        break;

    default:
        assert(!"Invalid search comparison type.");
        break;
    }
...
{
    return cmpFun(GetCurValueFromItemIndex<stepType, T>(itemIndex),
                  GetPrevValueFromItemIndex<stepType, T>(itemIndex),
                  param);
}
template<typename stepType, typename T>
...
bool Set_RS_Val()
{
    ...
    case 's':
        ...
        if ((rs_type_size == 'b' && rs_t == 's' && ((int) rs_val < -128 || (int) rs_val > 127))
            ...
            || (rs_type_size == 'w' && rs_t == 's'
                && ((int) rs_val < -32768 || (int) rs_val > 32767))
    ...
    default:
        ...
        break;
    case 'd':
        rs_param = ReadControlInt(IDC_EDIT_DIFFBY,
                                  (rs_c == 'r' || rs_c == 's') ? rs_type_size : 'd',
                                  (rs_c == 'r' || rs_c == 's') ? rs_t : (rs_c == 'a' ? 'h' : 's'),
                                  success);
        if (!success)
            return false;
        if ((int) rs_param < 0)
        ...
        break;
    case '%':
        rs_param = ReadControlInt(IDC_EDIT_MODBY,
                                  (rs_c == 'r' || rs_c == 's') ? rs_type_size : 'd',
                                  (rs_c == 'r' || rs_c == 's') ? rs_t : (rs_c == 'a' ? 'h' : 's'),
                                  success);
    ...
    // validate that rs_param fits in the comparison data type
    {
        ...
        if ((appliedSize == 'b' && appliedSign == 's'
             && ((int) rs_param < -128 || (int) rs_param > 127))
            || (appliedSize == 'b' && appliedSign != 's'
                && ((int) rs_param < 0 || (int) rs_param > 255))
            || (appliedSize == 'w' && appliedSign == 's'
                && ((int) rs_param < -32768 || (int) rs_param > 32767))
            || (appliedSize == 'w' && appliedSign != 's'
                && ((int) rs_param < 0 || (int) rs_param > 65535)))
            return false;
    }
...
#undef DO_SEARCH_2
#define DO_SEARCH_2(CmpFun, sf)                 \
    return CALL_WITH_T_SIZE_TYPES(sf,           \
                                  rs_type_size, \
                                  rs_t,         \
                                  noMisalign,   \
                                  CmpFun,       \
                                  ...
                                  rs_param);
    case 'r':
        DO_SEARCH(CompareRelativeAtItem);
        break;
    case 's':
        DO_SEARCH(CompareSpecificAtItem);
        break;

#undef DO_SEARCH_2
#define DO_SEARCH_2(CmpFun, sf)           \
    return CALL_WITH_T_STEP(sf,           \
                            rs_type_size, \
                            unsigned,     \
                            int,          \
                            noMisalign,   \
                            ...
                            rs_param);
    case 'a':
        DO_SEARCH(CompareAddressAtItem);
        break;
...
#define DO_SEARCH_2(CmpFun, sf)           \
    return CALL_WITH_T_STEP(sf,           \
                            ...
                            rs_param);
    case 'n':
        DO_SEARCH(CompareChangesAtItem);
        break;
    }
    return false;
}
...
bool WriteValueAtHardwareAddress(HWAddressType address,
                                 RSVal value,
                                 char sizeTypeID,
                                 char typeID,
                                 bool hookless)
...
void reset_address_info()
{
    ...
    EnterCriticalSection(&s_activeMemoryRegionsCS);
    s_activeMemoryRegionsBackup
        .clear(); // not necessary, but we'll take the time hit here instead of at the next thing that sets up an undo
    LeaveCriticalSection(&s_activeMemoryRegionsCS);
    if (s_prevValues)
...
void ReopenRamWindows() //Reopen them when a new Rom is loaded
{
    ...
    if (RamSearchClosed)
    {
        ...
        if (!RamSearchHWnd)
        {
            ...
            LRESULT CALLBACK RamSearchProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam);
            RamSearchHWnd =
                CreateDialog(hInst, MAKEINTRESOURCE(IDD_RAMSEARCH), hWnd, (DLGPROC) RamSearchProc);
        }
    }
    if (RamWatchClosed || AutoRWLoad)
    {
        ...
        if (!RamWatchHWnd)
        {
            ...
            RamWatchHWnd =
                CreateDialog(hInst, MAKEINTRESOURCE(IDD_RAMWATCH), hWnd, (DLGPROC) RamWatchProc);
        }
    }
...
void RefreshRamListSelectedCountControlStatus(HWND hDlg)
{
    ...
    if (selCount != prevSelCount)
    {
        ...
        {
            EnableWindow(GetDlgItem(hDlg, IDC_C_WATCH),
                         (selCount >= 1 && WatchCount < MAX_WATCH_COUNT) ? TRUE : FALSE);
            EnableWindow(GetDlgItem(hDlg, IDC_C_ADDCHEAT),
                         (selCount >= 1) ? /*TRUE*/ FALSE : FALSE);
...
void signal_new_size()
{
    ...
    unsigned int oldTopAddr = CALL_WITH_T_SIZE_TYPES(GetHardwareAddressFromItemIndex,
                                                     rs_last_type_size,
                                                     rs_t,
                                                     rs_last_no_misalign,
                                                     oldTopIndex);
    unsigned int oldSelectionAddr = CALL_WITH_T_SIZE_TYPES(GetHardwareAddressFromItemIndex,
                                                           rs_last_type_size,
                                                           rs_t,
                                                           rs_last_no_misalign,
                                                           oldSelectionIndex);
    ...
    if (numberOfItemsChanged)
    {
        ...
        for (int i = 0; i < selCount; ++i)
        {
            ...
            int addr = CALL_WITH_T_SIZE_TYPES(GetHardwareAddressFromItemIndex,
                                              rs_last_type_size,
                                              rs_t,
                                              rs_last_no_misalign,
                                              watchIndex);
    ...
    if (numberOfItemsChanged)
    {
        ...
        unsigned int newTopIndex = CALL_WITH_T_SIZE_TYPES(HardwareAddressToItemIndex,
                                                          rs_type_size,
                                                          rs_t,
                                                          noMisalign,
                                                          oldTopAddr);
        ...
        for (unsigned int i = 0; i < selHardwareAddrs.size(); i++)
        {
            ...
            int selRangeTop = CALL_WITH_T_SIZE_TYPES(HardwareAddressToItemIndex,
                                                     rs_type_size,
                                                     rs_t,
                                                     noMisalign,
                                                     range.addr);
            int selRangeBottom = -1;
            for (int endAddr = range.End() - 1; endAddr >= selRangeTop && selRangeBottom == -1;
                 endAddr--)
                selRangeBottom = CALL_WITH_T_SIZE_TYPES(HardwareAddressToItemIndex,
                                                        rs_type_size,
                                                        rs_t,
                                                        noMisalign,
                                                        endAddr);
...
void Update_RAM_Search() //keeps RAM values up to date in the search and watch windows
{
    ...
    if (Config::fastforward && (RamWatchHWnd || last_rs_possible > 10000 || Config::recoveringStale)
        && (Config::localTASflags.fastForwardFlags & /*FFMODE_RAMSKIP*/ 0x08))
    {
        static int count = 0;
    ...
    if (AutoSearch && !ResultCount)
    {
        ...
        {
            //			Clear_Sound_Buffer();
            int answer = MessageBoxW(RamSearchHWnd,
                                     L"Choosing Retry will reset the search once and continue "
                                     L"autosearching.\nChoose Ignore will reset the search "
                                     L"whenever necessary and continue autosearching.\nChoosing "
                                     L"Abort will reset the search once and stop autosearching.",
                                     L"Autosearch - out of results.",
    ...
    if (RamSearchHWnd)
    {
        ...
        else if (s_numChanges)
        {
            ...
            {
                int changeNum = CALL_WITH_T_SIZE_TYPES(GetNumChangesFromItemIndex,
                                                       rs_type_size,
                                                       rs_t,
                                                       noMisalign,
                                                       i); //s_numChanges[i];
...
LRESULT CALLBACK RamSearchProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INITDIALOG:
    {
        ...
        SendDlgItemMessage(hDlg,
                           IDC_C_AUTOSEARCH,
                           BM_SETCHECK,
                           AutoSearch ? BST_CHECKED : BST_UNCHECKED,
                           0);
        ...
        if (!noMisalign)
            SendDlgItemMessage(hDlg, IDC_MISALIGN, BM_SETCHECK, BST_CHECKED, 0);
    ...
    case WM_NOTIFY:
    {
        ...
        case LVN_GETDISPINFO:
        {
            ...
            case 1:
            {
                RSVal rsval = CALL_WITH_T_SIZE_TYPES((RSVal) GetCurValueFromItemIndex,
                                                     rs_type_size,
                                                     rs_t,
                                                     noMisalign,
                                                     iNum);
            ...
            case 2:
            {
                RSVal rsval = CALL_WITH_T_SIZE_TYPES((RSVal) GetPrevValueFromItemIndex,
                                                     rs_type_size,
                                                     rs_t,
                                                     noMisalign,
                                                     iNum);
            ...
            case 3:
            {
                int i = CALL_WITH_T_SIZE_TYPES(GetNumChangesFromItemIndex,
                                               rs_type_size,
                                               rs_t,
                                               noMisalign,
                                               iNum);
    ...
    case WM_COMMAND:
    {
        ...
        case IDC_C_RESET:
        {
            ...
            ListView_SetItemState(GetDlgItem(hDlg, IDC_RAMLIST),
                                  -1,
                                  0,
                                  LVIS_SELECTED); // deselect all
            //ListView_SetItemCount(GetDlgItem(hDlg,IDC_RAMLIST),ResultCount);
        ...
        invalid_field:
            MessageBoxW(RamSearchHWnd,
                        L"Invalid or out-of-bound entered value.",
                        L"Error",
                        MB_OK | MB_ICONSTOP);
        ...
        case IDC_C_WATCH:
        {
            ...
            while (watchItemIndex >= 0)
            {
                ...
                tempWatch.Address = CALL_WITH_T_SIZE_TYPES(GetHardwareAddressFromItemIndex,
                                                           rs_type_size,
                                                           rs_t,
                                                           noMisalign,
                                                           watchItemIndex);
                ...
                watchItemIndex =
                    ListView_GetNextItem(ramListControl, watchItemIndex, LVNI_SELECTED);
            }
        ...
        case IDC_C_ELIMINATE:
        {
            ...
            for (int i = 0, j = 1024; i < selCount; ++i, --j)
            {
                ...
                if (!j)
                    UpdateRamSearchProgressBar(i * 50 / selCount), j = 1024;
            }
            ...
            for (int i = 0, j = 16; i < numHardwareAddrRanges; ++i, --j)
            {
                ...
                if (!j)
                    UpdateRamSearchProgressBar(50 + (i * 50 / selCount)), j = 16;
            }
...
#define STATUS_STR L"%d Possibilit%s (%d Region%s)"
    ...
    if (poss <= 0)
        ...
        swprintf(Str_Tmp_RS,
                 ARRAYSIZE(Str_Tmp_RS),
                 HEADER_STR STATUS_STR,
                 poss,
                 poss == 1 ? L"y" : L"ies",
                 regions,
                 regions == 1 ? L"" : L"s");
    ...
    if (s_undoType != type)
    {
        if ((s_undoType != 2 && s_undoType != -1) != (type != 2 && type != -1))
            SendDlgItemMessageW(hDlg,
                                ...
                                WM_SETTEXT,
                                0,
                                (LPARAM)((type == 2 || type == -1) ? L"Redo" : L"Undo"));
...
void init_list_box(HWND Box,
                   LPCWSTR Strs[],
                   int numColumns,
                   int* columnWidths) //initializes the ram search and/or ram watch listbox
{
    LVCOLUMN Col;
...
void DeallocateRamSearch()
{
    ...
    s_itemIndexToRegionPointer = 0;
    EnterCriticalSection(&s_activeMemoryRegionsCS);
    MemoryList temp1;
    s_activeMemoryRegions.swap(temp1);
    MemoryList temp2;
    s_activeMemoryRegionsBackup.swap(temp2);
    LeaveCriticalSection(&s_activeMemoryRegionsCS);
}
